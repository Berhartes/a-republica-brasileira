/**
 * Script para processar blocos parlamentares (extração, transformação, carregamento e exportação)
...
import { blocosExporter } from './exportacao/blocos';
...
interface OpcoesProcessamento {
  ...
  limiteBlocos?: number; // Limite de blocos a processar (para testes)
}

/**
 * Função para processar o fluxo completo de blocos parlamentares com exportação automática
 * @param opcoes - Opções de processamento
 */
async function processarBlocosComExportacao(opcoes: OpcoesProcessamento): Promise<void> {
  try {
    logger.info('=== Iniciando processamento de blocos parlamentares com exportação ===');
    ...
    // Aplicar limite de blocos se definido (para testes rápidos)
    if (opcoes.limiteBlocos && opcoes.limiteBlocos > 0 && opcoes.limiteBlocos < dadosTransformados.blocos.length) {
      logger.info(`Aplicando limite de ${opcoes.limiteBlocos} blocos para teste rápido`);
      dadosTransformados.blocos = dadosTransformados.blocos.slice(0, opcoes.limiteBlocos);
      dadosTransformados.total = dadosTransformados.blocos.length;
    }
    ...
    logger.info('2.1.A Exportando blocos transformados para arquivos JSON');
    await blocosExporter.exportBlocos(dadosTransformados, legislaturaAtual);
    ...
    await blocosExporter.exportHistorico(dadosTransformados, legislaturaAtual);
    
    // 5. Exportação avançada (nova etapa)
    if (opcoes.exportar) {
      logger.info('5. Iniciando etapa de exportação avançada');
      await exportarDados(dadosTransformados.blocos, legislaturaAtual, opcoes.opcoesExportacao, tempoInicio);
    ...
    logger.info(`=== Processamento de blocos parlamentares concluído com sucesso em ${tempoTotal.toFixed(2)}s ===`);
    logger.info('NOTA: Os dados foram exportados para a pasta "dados_extraidos" na raiz do projeto.');
  } catch (error: any) {
    handleError(error, 'processarBlocosComExportacao');
    throw error;
  }
}

/**
 * Função para processar os blocos com opções de exportação definidas pela linha de comando
 */
function processarBlocosComExportacaoComandos(): void {
  const args = process.argv.slice(2);
  let legislatura: number | undefined;
  let exportar: boolean = false;
  let limiteBlocos: number | undefined = undefined;
  ...
  logger.info(`Configuração: Legislatura=${legislatura || 'atual'}, Exportar=${exportar}, Formato=${opcoesExportacao.formato}, Comprimir=${opcoesExportacao.comprimir}, Detalhamento=${opcoesExportacao.nivelDetalhamento}, Caminho=${opcoesExportacao.caminhoBase}${limiteBlocos ? `, Limite=${limiteBlocos} blocos` : ''}`);
  
  // Processar legislatura específica ou atual com exportação
  processarBlocosComExportacao({
    legislatura,
    exportar,
    opcoesExportacao,
    limiteBlocos
    ...
    .catch(error => {
      logger.error('Falha no processamento de blocos com exportação', error);
      process.exit(1);
    });
}
...
if (require.main === module) {
  processarBlocosComExportacaoComandos();
}

export { processarBlocosComExportacao };
