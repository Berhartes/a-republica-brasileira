...
export class DiscursosLoader {
   ...
   */
  async saveDiscurso(
    discursoData: DiscursoTransformado,
    ...
    try {
      ...
      if (!discursoData || !discursoData.codigo) {
        logger.error('Dados de discurso inválidos para salvamento');
        return {
      ...
      logger.info(`Salvando ${modoAtualizacao} do senador ${discursoData.senador.nome} (${discursoData.codigo}) ${legislaturaNumero ? `da legislatura ${legislaturaNumero}` : ''}`);
      ...
      if (apenasDiscursos) {
        ...
        logger.info(`Adicionando/atualizando apenas discursos para o senador ${discursoData.codigo}, preservando outros dados existentes`);
      } else if (apenasApartes) {
        // Adicionar apenas apartes
        dadosParaSalvar.apartes = discursoData.apartes;
        logger.info(`Adicionando/atualizando apenas apartes para o senador ${discursoData.codigo}, preservando outros dados existentes`);
      } else {
        // Adicionar ambos
        dadosParaSalvar.discursos = discursoData.discursos;
        dadosParaSalvar.apartes = discursoData.apartes;
        logger.info(`Atualizando discursos e apartes para o senador ${discursoData.codigo}`);
      }

      // Salvar no Firestore com merge: true para preservar outros campos existentes
      batchManager.set(discursoRef, dadosParaSalvar, { merge: true });

      // Commit das operações
      await batchManager.commit();

      logger.info(`${modoAtualizacao} do senador ${discursoData.codigo} salvos com sucesso`);
      ...
      logger.error(`Erro ao salvar discursos do senador ${discursoData?.codigo || 'desconhecido'}: ${error.message}`);
      return {
        timestamp: new Date().toISOString(),
  ...
  async saveMultiplosDiscursos(
    ...
    try {
      ...
      logger.info(`Salvando ${modoAtualizacao} de ${discursos.length} senadores ${legislaturaNumero ? `da legislatura ${legislaturaNumero}` : ''}`);
      ...
      for (const [indice, lote] of lotes.entries()) {
        logger.info(`Processando lote ${indice + 1}/${lotes.length} (${lote.length} senadores)`);
        ...
        for (const discurso of lote) {
          try {
            ...
            if (!discurso || !discurso.codigo) {
              logger.warn(`Discurso inválido encontrado no lote ${indice + 1}, pulando...`);
              falhas++;
              continue;
            }
            ...
            if (apenasDiscursos) {
              ...
              logger.info(`Adicionando/atualizando apenas discursos para o senador ${discurso.codigo}, preservando outros dados existentes`);
            } else if (apenasApartes) {
              // Adicionar apenas apartes
              dadosParaSalvar.apartes = discurso.apartes;
              logger.info(`Adicionando/atualizando apenas apartes para o senador ${discurso.codigo}, preservando outros dados existentes`);
            } else {
              // Adicionar ambos
              dadosParaSalvar.discursos = discurso.discursos;
              dadosParaSalvar.apartes = discurso.apartes;
              logger.info(`Atualizando discursos e apartes para o senador ${discurso.codigo}`);
            }
            ...
            logger.warn(`Erro ao processar discursos do senador ${discurso?.codigo || 'desconhecido'} no lote ${indice + 1}: ${error.message}`);
            falhas++;
          }
        }
      ...
      logger.info(`Salvamento de discursos concluído: ${sucessos} sucessos, ${falhas} falhas`);
      ...
      logger.error(`Erro ao salvar múltiplos discursos: ${error.message}`);
      return {
        timestamp: new Date().toISOString(),
        total: discursos.length,
        sucessos: 0,
...
export const discursosLoader = new DiscursosLoader();
