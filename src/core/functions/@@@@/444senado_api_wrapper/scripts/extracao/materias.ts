...
/**
 * Interface para resultado de extração de matérias
 */
 ...
 */
export class MateriasExtractor {
  /**
   ...
   */
  async extractMaterias(codigoSenador: string | number): Promise<MateriaResult> {
    try {
      logger.info(`Extraindo matérias legislativas do senador código ${codigoSenador}`);
      ...
      if (!dadosBasicos || !dadosBasicos.dados) {
        logger.warn(`Dados básicos não obtidos para o senador ${codigoSenador}, abortando extração de matérias`);
        return {
      ...
      const materiaCompleta: MateriaResult = {
        timestamp: new Date().toISOString(),
        codigo: codigoSenador,
        dadosBasicos: dadosBasicos,
      ...
      logger.error(`Erro ao extrair matérias do senador ${codigoSenador}: ${error.message}`);
      return {
        timestamp: new Date().toISOString(),
        codigo: codigoSenador,
        erro: error.message
  ...
  async extractAutorias(
    ...
    try {
      if (dataInicio && dataFim) {
        logger.info(`Extraindo autorias do senador código ${codigoSenador} no período de ${dataInicio} a ${dataFim}`);
      } else {
        logger.info(`Extraindo autorias do senador código ${codigoSenador} (sem período específico)`);
      }

      // Construir parâmetros da requisição
      const params: Record<string, any> = { ...endpoints.PROCESSO.AUTORIAS.PARAMS };
      ...
      logger.info(`Fazendo requisição para ${endpoints.PROCESSO.AUTORIAS.PATH} com parâmetros: ${JSON.stringify(params)}`);

      // Fazer a requisição
      const response = await withRetry(
        async () => api.get(endpoints.PROCESSO.AUTORIAS.PATH, params),
        endpoints.REQUEST.RETRY_ATTEMPTS,
        endpoints.REQUEST.RETRY_DELAY,
        `Extração autorias do senador ${codigoSenador}`
      );
      ...
      return {
        timestamp: new Date().toISOString(),
        origem: `${endpoints.PROCESSO.AUTORIAS.PATH}?codigoParlamentarAutor=${codigoSenador}${dataInicio ? `&dataInicioApresentacao=${dataInicio}` : ''}${dataFim ? `&dataFimApresentacao=${dataFim}` : ''}`,
        dados: response,
        metadados: {
      ...
      logger.error(`Erro ao extrair autorias do senador ${codigoSenador}: ${error.message}`);
      return {
        timestamp: new Date().toISOString(),
        origem: `${endpoints.PROCESSO.AUTORIAS.PATH}?codigoParlamentarAutor=${codigoSenador}${dataInicio ? `&dataInicioApresentacao=${dataInicio}` : ''}${dataFim ? `&dataFimApresentacao=${dataFim}` : ''}`,
        dados: null,
  ...
  async extractRelatorias(
    ...
    try {
      if (dataInicio && dataFim) {
        logger.info(`Extraindo relatorias do senador código ${codigoSenador} no período de ${dataInicio} a ${dataFim}`);
      } else {
        logger.info(`Extraindo relatorias do senador código ${codigoSenador} (sem período específico)`);
      }

      // Construir parâmetros da requisição
      const params: Record<string, any> = { ...endpoints.PROCESSO.RELATORIAS.PARAMS };
      ...
      // Fazer a requisição
      const response = await withRetry(
        async () => api.get(endpoints.PROCESSO.RELATORIAS.PATH, params),
        endpoints.REQUEST.RETRY_ATTEMPTS,
        endpoints.REQUEST.RETRY_DELAY,
        `Extração relatorias do senador ${codigoSenador}`
      );

      return {
        timestamp: new Date().toISOString(),
        origem: `${endpoints.PROCESSO.RELATORIAS.PATH}?codigoParlamentar=${codigoSenador}${dataInicio ? `&dataInicio=${dataInicio}` : ''}${dataFim ? `&dataFim=${dataFim}` : ''}`,
        dados: response,
        metadados: {}
      };
    } catch (error: any) {
      logger.error(`Erro ao extrair relatorias do senador ${codigoSenador}: ${error.message}`);
      return {
        timestamp: new Date().toISOString(),
        origem: `${endpoints.PROCESSO.RELATORIAS.PATH}?codigoParlamentar=${codigoSenador}${dataInicio ? `&dataInicio=${dataInicio}` : ''}${dataFim ? `&dataFim=${dataFim}` : ''}`,
        dados: null,
   ...
   */
  async extractAutoriasPorPeriodoMandato(
    codigoSenador: string | number,
    dataInicioMandato: string,
    dataFimMandato: string
  ): Promise<any[]> {
    try {
      logger.info(`Extraindo autorias do senador ${codigoSenador} durante o mandato (${dataInicioMandato} a ${dataFimMandato})`);
      ...
      logger.info(`Período dividido em ${intervalos.length} intervalos de 360 dias para autorias`);

      // Extrair autorias para cada intervalo
      const resultados: any[] = [];

      for (const [index, intervalo] of intervalos.entries()) {
        logger.info(`Processando intervalo de autorias ${index + 1}/${intervalos.length}: ${intervalo.inicio.toISOString().slice(0, 10)} a ${intervalo.fim.toISOString().slice(0, 10)}`);
        ...
        const resultado = await this.extractAutorias(
          codigoSenador,
          dataInicioFormatada,
          dataFimFormatada
        );
        ...
        if (index < intervalos.length - 1) {
          logger.debug(`Aguardando 1 segundo antes da próxima requisição de autorias...`);
          await new Promise(resolve => setTimeout(resolve, 1000));
        }
      }

      logger.info(`Extração de autorias por período concluída: ${resultados.length} intervalos com dados`);
      return resultados;
    } catch (error: any) {
      logger.error(`Erro ao extrair autorias por período do senador ${codigoSenador}: ${error.message}`);
      throw error;
    }
  }
   ...
   */
  async extractRelatoriasPorPeriodoMandato(
    codigoSenador: string | number,
    dataInicioMandato: string,
    dataFimMandato: string
  ): Promise<any[]> {
    try {
      logger.info(`Extraindo relatorias do senador ${codigoSenador} durante o mandato (${dataInicioMandato} a ${dataFimMandato})`);
      ...
      // Dividir o período em intervalos de 360 dias
      const intervalos = this.gerarIntervalos360Dias(dataInicio, dataFimEfetiva);
      logger.info(`Período dividido em ${intervalos.length} intervalos de 360 dias para relatorias`);

      // Extrair relatorias para cada intervalo
      const resultados: any[] = [];

      for (const [index, intervalo] of intervalos.entries()) {
        logger.info(`Processando intervalo de relatorias ${index + 1}/${intervalos.length}: ${intervalo.inicio.toISOString().slice(0, 10)} a ${intervalo.fim.toISOString().slice(0, 10)}`);
        ...
        const resultado = await this.extractRelatorias(
          codigoSenador,
          dataInicioFormatada,
          dataFimFormatada
        );
        ...
        // Pausa entre requisições para não sobrecarregar a API
        if (index < intervalos.length - 1) {
          logger.debug(`Aguardando 1 segundo antes da próxima requisição de relatorias...`);
          await new Promise(resolve => setTimeout(resolve, 1000));
        }
      }

      logger.info(`Extração de relatorias por período concluída: ${resultados.length} intervalos com dados`);
      return resultados;
    } catch (error: any) {
      logger.error(`Erro ao extrair relatorias por período do senador ${codigoSenador}: ${error.message}`);
      throw error;
    }
  }
   ...
   */
  consolidarResultadosAutorias(resultados: any[]): any {
    try {
      logger.info(`Consolidando ${resultados.length} resultados de autorias`);

      // Verificar se há resultados para consolidar
      if (!resultados || resultados.length === 0) {
        logger.warn('Nenhum resultado de autorias para consolidar');
        return null;
      }

      // Extrair todas as autorias de todos os períodos
      const todasAutorias: any[] = [];
      ...
      for (const resultado of resultados) {
        ...
        if (!processos) {
          logger.warn('Nenhum processo encontrado nos dados. Estrutura completa:', resultado.dados);
          continue;
        }

        // Garantir que processos seja um array
        const processosArray = Array.isArray(processos) ? processos : [processos];
        logger.info(`Adicionando ${processosArray.length} processos ao consolidado`);
        todasAutorias.push(...processosArray);
      }

      logger.info(`Consolidação concluída: ${todasAutorias.length} autorias encontradas`);
      ...
      logger.error(`Erro ao consolidar resultados de autorias: ${error.message}`);
      return null;
    }
  }
   ...
   * @returns Dados consolidados
   */
  consolidarResultadosRelatorias(resultados: any[]): any {
    try {
      logger.info(`Consolidando ${resultados.length} resultados de relatorias`);

      // Verificar se há resultados para consolidar
      if (!resultados || resultados.length === 0) {
        logger.warn('Nenhum resultado de relatorias para consolidar');
        return null;
      }

      // Extrair todas as relatorias de todos os períodos
      const todasRelatorias: any[] = [];

      for (const resultado of resultados) {
        if (!resultado || !resultado.dados) continue;

        // Log para depuração da estrutura completa
        logger.info(`Estrutura de dados do resultado de relatorias: ${JSON.stringify(Object.keys(resultado.dados), null, 2)}`);
        ...
        // Caminho 6: Objeto com array
          ...
          for (const key in resultado.dados) {
            if (Array.isArray(resultado.dados[key])) {
              ...
              logger.info(`Relatorias encontradas na propriedade ${key}`);
              break;
            }
          }
        }
        ...
        if (!relatorias) {
          logger.warn('Nenhuma relatoria encontrada nos dados. Estrutura completa:', JSON.stringify(resultado.dados, null, 2));
          continue;
        }

        // Garantir que relatorias seja um array
        const relatoriasArray = Array.isArray(relatorias) ? relatorias : [relatorias];
        logger.info(`Adicionando ${relatoriasArray.length} relatorias ao consolidado`);
        todasRelatorias.push(...relatoriasArray);
      }

      logger.info(`Consolidação concluída: ${todasRelatorias.length} relatorias encontradas`);
      ...
      logger.error(`Erro ao consolidar resultados de relatorias: ${error.message}`);
      return null;
    }
  }
   ...
   */
  private gerarIntervalos360Dias(dataInicio: Date, dataFim: Date): Array<{inicio: Date, fim: Date}> {
...
export const materiasExtractor = new MateriasExtractor();
