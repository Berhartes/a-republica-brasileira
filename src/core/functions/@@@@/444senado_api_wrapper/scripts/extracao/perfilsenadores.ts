...
export interface PerfilCompletoResult {
  ...
  licencas?: any;
  profissao?: any;
  liderancas?: any; // Adicionado campo para lideranças
  erro?: string;
}
...
export class PerfilSenadoresExtractor {
  ...
  async extractPerfilCompleto(codigoSenador: string | number): Promise<PerfilCompletoResult> {
    try {
      ...
      const [
        ...
        licencas,
        profissao,
        liderancas // Adicionado lideranças
      ...
      const perfilCompleto: PerfilCompletoResult = {
        ...
        liderancas: liderancas.status === 'fulfilled' ? liderancas.value : {
          timestamp: new Date().toISOString(),
          origem: endpoints.SENADORES.LIDERANCAS.PATH,
          dados: null,
      ...
      if (response.DetalheParlamentar.Parlamentar) {
        ...
        if (parlamentar.Telefones) {
          ...
          if (parlamentar.Telefones.Telefone) {
            ...
            telefones.forEach((tel: any, index: number) => {
              logger.debug(`Telefone ${index + 1}: ${JSON.stringify(tel)}`);
            });
          }
        }
        ...
        if (parlamentar.IdentificacaoParlamentar && parlamentar.IdentificacaoParlamentar.Telefones) {
          logger.debug(`Telefones encontrados na identificação do parlamentar: ${JSON.stringify(parlamentar.IdentificacaoParlamentar.Telefones)}`);
          ...
          if (parlamentar.IdentificacaoParlamentar.Telefones.Telefone) {
            ...
            telefones.forEach((tel: any, index: number) => {
              logger.debug(`Telefone na identificação ${index + 1}: ${JSON.stringify(tel)}`);
            });
          }
        }
        ...
        if (parlamentar.IdentificacaoParlamentar && parlamentar.IdentificacaoParlamentar.SexoParlamentar) {
          logger.debug(`Gênero do senador ${codigoSenador}: ${parlamentar.IdentificacaoParlamentar.SexoParlamentar}`);
        }
      }
   ...
   */
  async extractApartes(
    codigoSenador: string | number,
    dataInicio?: string,
    dataFim?: string
  ): Promise<PerfilSenadorResult> {
    try {
      if (dataInicio && dataFim) {
        logger.info(`Extraindo apartes do senador código ${codigoSenador} no período de ${dataInicio} a ${dataFim}`);
      } else {
        logger.info(`Extraindo apartes do senador código ${codigoSenador} (sem período específico)`);
      }
      ...
      const urlCompleta = `${endpoints.BASE_URL}${endpoint}?${Object.entries(params)
        .map(([key, value]) => `${key}=${value}`)
        .join('&')}`;
      logger.debug(`URL completa para apartes: ${urlCompleta}`);
      ...
      logger.debug(`Estrutura da resposta de apartes: ${JSON.stringify(Object.keys(response))}`);

      // Verificar estrutura da resposta
      if (!response.ApartesParlamentar) {
        logger.warn(`Estrutura de apartes não encontrada para o senador ${codigoSenador}`);
        logger.debug(`Conteúdo da resposta: ${JSON.stringify(response).substring(0, 500)}...`);
        return {
          timestamp: new Date().toISOString(),
          origem: endpoint,
          dados: null,
      ...
      if (response.ApartesParlamentar.Parlamentar &&
        ...
        if (apartes) {
          ...
          logger.info(`Encontrados ${numApartes} apartes para o senador ${codigoSenador}`);
        } else {
          logger.info(`Nenhum aparte encontrado para o senador ${codigoSenador}`);
          logger.debug(`Estrutura de Apartes: ${JSON.stringify(response.ApartesParlamentar.Parlamentar.Apartes)}`);
        }
      } else {
        logger.info(`Estrutura de Apartes não encontrada para o senador ${codigoSenador}`);
        if (response.ApartesParlamentar.Parlamentar) {
          logger.debug(`Chaves em Parlamentar: ${JSON.stringify(Object.keys(response.ApartesParlamentar.Parlamentar))}`);
        }
      }
      ...
      if (response.ApartesParlamentar) {
        logger.debug(`Estrutura de ApartesParlamentar: ${JSON.stringify(Object.keys(response.ApartesParlamentar))}`);

        // Verificar se a resposta contém o campo Parlamentar
        if (response.ApartesParlamentar.Parlamentar) {
          logger.debug(`Estrutura de Parlamentar: ${JSON.stringify(Object.keys(response.ApartesParlamentar.Parlamentar))}`);

          // Verificar se a resposta contém o campo Apartes
          if (response.ApartesParlamentar.Parlamentar.Apartes) {
            logger.debug(`Estrutura de Apartes: ${JSON.stringify(Object.keys(response.ApartesParlamentar.Parlamentar.Apartes))}`);

            // Verificar se a resposta contém o campo Aparte
            if (response.ApartesParlamentar.Parlamentar.Apartes.Aparte) {
              const apartes = response.ApartesParlamentar.Parlamentar.Apartes.Aparte;
              logger.debug(`Apartes encontrados: ${Array.isArray(apartes) ? apartes.length : 1}`);

              // Se for um único aparte, verificar a estrutura
              if (!Array.isArray(apartes)) {
                logger.debug(`Estrutura do aparte: ${JSON.stringify(Object.keys(apartes))}`);

                // Verificar se o aparte contém o campo TextoResumo
                if (apartes.TextoResumo) {
                  logger.debug(`TextoResumo encontrado: ${apartes.TextoResumo}`);
                } else {
                  logger.debug(`TextoResumo não encontrado no aparte`);
                  // Listar todos os campos do aparte para debug
                  Object.keys(apartes).forEach(key => {
                    logger.debug(`Campo ${key}: ${typeof apartes[key] === 'string' ? apartes[key] : JSON.stringify(apartes[key])}`);
                  });
                }
              } else if (apartes.length > 0) {
                // Se for um array, verificar a estrutura do primeiro aparte
                logger.debug(`Estrutura do primeiro aparte: ${JSON.stringify(Object.keys(apartes[0]))}`);

                // Verificar se o primeiro aparte contém o campo TextoResumo
                if (apartes[0].TextoResumo) {
                  logger.debug(`TextoResumo encontrado no primeiro aparte: ${apartes[0].TextoResumo}`);
                } else {
                  logger.debug(`TextoResumo não encontrado no primeiro aparte`);
                  // Listar todos os campos do primeiro aparte para debug
                  Object.keys(apartes[0]).forEach(key => {
                    logger.debug(`Campo ${key}: ${typeof apartes[0][key] === 'string' ? apartes[0][key] : JSON.stringify(apartes[0][key])}`);
                  });
                }
              }
            } else {
              logger.debug(`Aparte não encontrado na resposta`);
            }
          } else {
            logger.debug(`Apartes não encontrado na resposta`);
          }
        } else {
          logger.debug(`Parlamentar não encontrado na resposta`);
        }
      } else {
        logger.debug(`ApartesParlamentar não encontrado na resposta`);
      }
   ...
   */
  async extractApartesPorPeriodoMandato(
    codigoSenador: string | number,
    ...
    try {
      logger.info(`Extraindo apartes do senador ${codigoSenador} durante o mandato (${dataInicioMandato} a ${dataFimMandato}) com concorrência ${concurrency}`);
      ...
      logger.info(`Período dividido em ${intervalos.length} intervalos de 30 dias para apartes`);

      // Extrair apartes para cada intervalo com concorrência limitada
      const resultados: PerfilSenadorResult[] = [];
      ...
      for (const [chunkIndex, chunk] of chunks.entries()) {
        logger.info(`Processando chunk de apartes ${chunkIndex + 1}/${chunks.length} (${chunk.length} intervalos)`);

        // Processar intervalos do chunk atual em paralelo
        const chunkResultados = await Promise.all(
          chunk.map(async (intervalo, idx) => {
            const intervaloIndex = chunkIndex * concurrency + idx;
            logger.info(`Processando intervalo de apartes ${intervaloIndex + 1}/${intervalos.length}: ${intervalo.inicio.toISOString().slice(0, 10)} a ${intervalo.fim.toISOString().slice(0, 10)}`);
            ...
            try {
              ...
              const resultado = await this.extractApartes(
                codigoSenador,
                dataInicioFormatada,
                dataFimFormatada
              );
              ...
              logger.error(`Erro ao extrair apartes do intervalo ${intervaloIndex + 1}: ${error.message}`);
              return null;
            }
          })
        );
        ...
        resultados.push(...chunkResultados.filter(Boolean) as PerfilSenadorResult[]);

        // Pausa entre chunks para não sobrecarregar a API
        if (chunkIndex < chunks.length - 1) {
          logger.info(`Aguardando 2 segundos antes de processar o próximo chunk de intervalos de apartes...`);
          await new Promise(resolve => setTimeout(resolve, 2000));
        }
      }

      logger.info(`Extração de apartes por período concluída: ${resultados.length} intervalos com dados`);
      return resultados;
    } catch (error: any) {
      logger.error(`Erro ao extrair apartes por período do senador ${codigoSenador}: ${error.message}`);
      throw error;
    }
  }
   ...
   */
  async extractLiderancas(codigoSenador: string | number): Promise<PerfilSenadorResult> {
    try {
      logger.info(`Extraindo lideranças do senador código ${codigoSenador}`);

      // Usar o endpoint de composição/lideranca com filtro por parlamentar
      const endpointConfig = endpoints.SENADORES.LIDERANCAS;
      ...
      logger.debug(`URL para lideranças: ${endpointConfig.PATH}`);
      logger.debug(`Parâmetros para lideranças: ${JSON.stringify(params)}`);

      // Construir a URL completa para verificação
      const urlCompleta = `${endpoints.BASE_URL}${endpointConfig.PATH}?${Object.entries(params)
        .map(([key, value]) => `${key}=${value}`)
        .join('&')}`;
      logger.debug(`URL completa para lideranças: ${urlCompleta}`);

      const response = await withRetry(
        async () => api.get(endpointConfig.PATH, params),
        endpoints.REQUEST.RETRY_ATTEMPTS,
        endpoints.REQUEST.RETRY_DELAY,
        `Extração lideranças do senador ${codigoSenador}`
      );

      // Verificação rigorosa da resposta
      if (!response) {
        logger.warn(`Resposta vazia ao buscar lideranças do senador ${codigoSenador}`);
        return {
          timestamp: new Date().toISOString(),
      ...
      if (!dadosLiderancas && Object.keys(response).length > 0) {
        logger.debug(`Usando a resposta completa como dados de lideranças`);
        dadosLiderancas = response;
      }

      // Adicionar log para verificar a estrutura dos dados de lideranças
      if (dadosLiderancas) {
        logger.debug(`Estrutura de lideranças para o senador ${codigoSenador}: ${JSON.stringify(Object.keys(dadosLiderancas))}`);
      }
      ...
      logger.error(`Erro ao extrair lideranças do senador ${codigoSenador}: ${error.message}`);
   ...
   */
  async extractDiscursos(codigoSenador: string | number): Promise<DiscursoResult> {
    try {
      ...
      // 3. Consolidar todos os dados
      const discursoCompleto: DiscursoResult = {
        timestamp: new Date().toISOString(),
        codigo: codigoSenador,
        dadosBasicos: dadosBasicos,
        discursos: discursos.status === 'fulfilled' ? discursos.value : {
          timestamp: new Date().toISOString(),
          origem: api.replacePath(endpoints.SENADORES.DISCURSOS.PATH, { codigo: codigoSenador.toString() }),
          dados: null,
          erro: discursos.status === 'rejected' ? discursos.reason?.message : 'Informação não disponível'
        }
      };

      logger.info(`Discursos do senador ${codigoSenador} extraídos com sucesso`);
  ...
  async extractDiscursosDetalhados(
    ...
    try {
      if (dataInicio && dataFim) {
        logger.info(`Extraindo discursos detalhados do senador código ${codigoSenador} no período de ${dataInicio} a ${dataFim}`);
      } else {
        logger.info(`Extraindo discursos detalhados do senador código ${codigoSenador} (sem período específico)`);
      }

      const endpointConfig = endpoints.SENADORES.DISCURSOS;
      const endpoint = api.replacePath(endpointConfig.PATH, { codigo: codigoSenador.toString() });
      ...
      const response = await withRetry(
        ...
        endpoints.REQUEST.RETRY_ATTEMPTS,
        endpoints.REQUEST.RETRY_DELAY,
        `Extração discursos detalhados do senador ${codigoSenador}`
      );

      // Verificação rigorosa da resposta
      if (!response) {
        logger.warn(`Resposta vazia ao buscar discursos detalhados do senador ${codigoSenador}`);
        return {
          timestamp: new Date().toISOString(),
          origem: endpoint,
          dados: null,
      ...
      logger.error(`Erro ao extrair discursos detalhados do senador ${codigoSenador}: ${error.message}`);
  ...
  async extractDiscursosPorPeriodoMandato(
    ...
    try {
      logger.info(`Extraindo discursos do senador ${codigoSenador} durante o mandato (${dataInicioMandato} a ${dataFimMandato}) com concorrência ${concurrency}`);
      ...
      // Dividir o período em intervalos de 30 dias
      const intervalos = this.gerarIntervalos30Dias(dataInicio, dataFimEfetiva);
      logger.info(`Período dividido em ${intervalos.length} intervalos de 30 dias`);
      ...
      // Processar cada chunk de intervalos em paralelo
      for (const [chunkIndex, chunk] of chunks.entries()) {
        logger.info(`Processando chunk ${chunkIndex + 1}/${chunks.length} (${chunk.length} intervalos)`);

        // Processar intervalos do chunk atual em paralelo
        const chunkResultados = await Promise.all(
          chunk.map(async (intervalo, idx) => {
            const intervaloIndex = chunkIndex * concurrency + idx;
            logger.info(`Processando intervalo ${intervaloIndex + 1}/${intervalos.length}: ${intervalo.inicio.toISOString().slice(0, 10)} a ${intervalo.fim.toISOString().slice(0, 10)}`);
            ...
            try {
              ...
              logger.error(`Erro ao extrair discursos do intervalo ${intervaloIndex + 1}: ${error.message}`);
              return null;
            }
          })
        );
        ...
        // Pausa entre chunks para não sobrecarregar a API
        if (chunkIndex < chunks.length - 1) {
          logger.info(`Aguardando 2 segundos antes de processar o próximo chunk de intervalos...`);
          await new Promise(resolve => setTimeout(resolve, 2000));
        }
      }
   ...
   */
  private gerarIntervalos30Dias(dataInicio: Date, dataFim: Date): Array<{inicio: Date, fim: Date}> {
   ...
   */
  async extractMultiplosDiscursos(
    codigosSenadores: (string | number)[],
    concurrency: number = 3,
    maxRetries: number = 3
  ): Promise<DiscursoResult[]> {
    try {
      logger.info(`Extraindo discursos de ${codigosSenadores.length} senadores (concorrência: ${concurrency})`);

      // Limitar concorrência para não sobrecarregar a API
      const discursos: DiscursoResult[] = [];
      const chunks = [];
      ...
      for (const [index, chunk] of chunks.entries()) {
        ...
        const chunkDiscursos = await Promise.all(
          chunk.map(async (codigo) => {
            try {
              // Tentar extrair com retry
              return await withRetry(
                async () => this.extractDiscursos(codigo),
                maxRetries,
                1000, // 1 segundo entre tentativas
                `Extração discursos do senador ${codigo}`
              );
            } catch (error: any) {
              logger.error(`Falha ao extrair discursos do senador ${codigo} após ${maxRetries} tentativas: ${error.message}`);
        ...
        // Mostrar progresso
        logger.info(`Progresso: ${Math.min(discursos.length, codigosSenadores.length)}/${codigosSenadores.length} senadores`);
      ...
      logger.error(`Erro ao extrair múltiplos discursos: ${error.message}`);
      throw error;
    }
  }
}
