...
import { LiderancasProcessor } from '../processors/liderancas.processor';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const cli = new ETLCommandParser(
      'senado:liderancas',
      'Processa lideranças parlamentares do Senado Federal'
    );

    // Adicionar opções customizadas específicas deste script
    cli.addCustomOption('--incluir-membros', () => true);
    cli.addCustomOption('--tipo-lideranca', (value: string) => value.toUpperCase());
    ...
    // Criar processador
    const processor = new LiderancasProcessor(options);
...
`);
  ...
  if (resultado.detalhes && Array.isArray(resultado.detalhes)) {
    ...
    const liderancas = resultado.detalhes.find((d: any) => d.id === 'liderancas');
    const referencias = resultado.detalhes.find((d: any) => d.id === 'referencias');
...
export { main as processarLiderancas };
export default main;
