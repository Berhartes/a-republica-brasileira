...
import { MateriasProcessor } from '../processors/materias.processor';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const cli = new ETLCommandParser(
      'senado:materias',
      'Processa matérias legislativas de senadores do Senado Federal'
    );
    ...
    cli.addCustomOption('--tipo-materia', (value: string) => value.toUpperCase());
    ...
    // Criar processador
    const processor = new MateriasProcessor(options);
    ...
    logger.info('Iniciando processamento de matérias legislativas...');
...
`);
  ...
  if (resultado.detalhes && Array.isArray(resultado.detalhes)) {
    ...
    const materias = resultado.detalhes.find((d: any) => d.id === 'materias' || d.id === 'materias_completas');
    const tipos = resultado.detalhes.filter((d: any) => d.id?.startsWith('tipo_'));
...
export { main as processarMateriasLegislativas };
export default main;
