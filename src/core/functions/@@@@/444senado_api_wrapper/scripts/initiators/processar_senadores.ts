...
import { SenadoresProcessor } from '../processors/senadores.processor';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const cli = new ETLCommandParser(
      'senado:senadores',
      'Processa senadores em exercício do Senado Federal'
    );
    ...
    // Criar processador
    const processor = new SenadoresProcessor(options);
...
`);
  ...
  if (resultado.detalhes && Array.isArray(resultado.detalhes)) {
    ...
    const senadores = resultado.detalhes.find((d: any) => d.id === 'senadores');
    const partidos = resultado.detalhes.filter((d: any) => d.id?.startsWith('partido_'));
    const ufs = resultado.detalhes.filter((d: any) => d.id?.startsWith('uf_'));
...
export { main as processarSenadores };
export default main;
