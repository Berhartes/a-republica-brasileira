...
import { blocosExtractor } from '../extracao/blocos';
import { blocosTransformer } from '../transformacao/blocos';
import { blocosLoader } from '../carregamento/blocos';
...
export class BlocosProcessor extends ETLProcessor<ExtractedData, TransformedData> {
  ...
  async extract(): Promise<ExtractedData> {
    ...
    const resultadoBlocos = await blocosExtractor.extractBlocosLegislatura(legislatura);
  ...
  async transform(data: ExtractedData): Promise<TransformedData> {
    ...
    for (const bloco of data.blocos) {
      try {
        const transformado = blocosTransformer.transformBloco(bloco);
        if (transformado) {
          ...
          if (membrosBloco) {
            const membrosTransf = membrosBloco.map(m => 
              blocosTransformer.transformMembroBloco(m, bloco.CodigoBloco)
            ).filter(Boolean);
  ...
  private async extrairMembros(blocos: any[]): Promise<Map<string, any[]>> {
    ...
    for (const bloco of blocos) {
      try {
        ...
        const resultadoMembros = await blocosExtractor.extractMembrosBloco(
          bloco.CodigoBloco,
  ...
  private async salvarNoFirestore(data: TransformedData): Promise<BatchResult> {
    ...
    try {
      // Salvar blocos
      const resultadoBlocos = await blocosLoader.saveMultiplosBlocos(
      ...
      for (const [blocoId, membros] of data.membrosTransformados) {
        try {
          const resultado = await blocosLoader.saveMembrosBloco(
            membros,
            blocoId,
            data.legislatura
          );
