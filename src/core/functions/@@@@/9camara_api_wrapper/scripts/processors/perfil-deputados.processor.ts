...
import { FirestoreBatchManager } from '../utils/storage/firestore/config'; // Importar FirestoreBatchManager
  ...
  async load(data: TransformedData): Promise<any> { // Assinatura original do método load
    ...
    try {
      ...
      if (this.rawPerfisCompletos && this.rawPerfisCompletos.length > 0) {
        for (const rawPerfil of this.rawPerfisCompletos) {
          if (rawPerfil && rawPerfil.id) {
            const { collectionPath, documentId } = parseFirestorePath('congressoNacional/CamaraDeputados/perfil', rawPerfil.id.toString());
      ...
      for (const perfil of data.perfis) { // data.perfis são os transformados
        ...
        const { collectionPath, documentId } = parseFirestorePath(`congressoNacional/CamaraDeputados/legislatura/${idLegislaturaParaSalvar}/deputados`, perfil.id.toString());
      ...
      const { collectionPath: metaCol, documentId: metaId } = parseFirestorePath(metadataCollection, metadataId);
