...
import { etlConfig } from '../config/etl.config'; // Remover processadorConfigs
...
export class DespesasDeputadosProcessor extends ETLProcessor<ExtractedData, TransformedData> {
  ...
  async load(data: TransformedData): Promise<ETLResult> {
    ...
    let batchResults: BatchResult | undefined; // Pode não ser definido se houver erro antes do commit
    const modoAtualizacao = this.context.options.atualizar || false;
      ...
      for (const [deputadoId, novasDespesasDoDeputado] of Object.entries(despesasPorDeputado)) {
        ...
        for (const [ano, novasDespesasDoAno] of Object.entries(novasDespesasAgrupadasPorAno)) {
          ...
          if (modoAtualizacao) {
            ...
            try {
              const docRef = db.doc(pathDespesasAno);
          ...
          const { collectionPath: colPath, documentId: docId } = parseFirestorePath(basePathDeputado, ano);
        ...
        if (todasAsDespesasDoDeputadoParaEstatisticas.length > 0) {
            ...
            const { collectionPath: colPathMain, documentId: docIdMain } = parseFirestorePath(`congressoNacional/CamaraDeputados/despesas`, deputadoId);
