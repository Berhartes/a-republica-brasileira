...
export async function obterLegislaturaAtual(data?: string): Promise<Legislatura | null> {
  ...
  logger.info(`Obtendo legislatura para a data ${data}`);
    ...
    try {
      ...
      const endpoint = api.replacePath(endpoints.LEGISLATURAS.LISTA.PATH, { data }); // Corrigido para LISTA
      
      // Fazer a requisição
      // O endpoint LISTA espera o parâmetro 'data' para filtrar
      const params = { ...endpoints.LEGISLATURAS.LISTA.PARAMS, data };
      const response = await withRetry(
        async () => api.get(endpoint, params), // Corrigido para LISTA e passando params com data
        endpoints.REQUEST.RETRY_ATTEMPTS,
        endpoints.REQUEST.RETRY_DELAY,
        `Extração legislatura atual para data ${data}`
      );
