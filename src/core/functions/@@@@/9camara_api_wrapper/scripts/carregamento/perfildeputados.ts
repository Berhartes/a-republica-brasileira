...
import { db, FirestoreBatchManager } from '../utils/storage/firestore/config'; // Admin SDK e BatchManager
...
export class PerfilDeputadosLoader {
  ...
  async salvarDeputadosLegislatura(
    ...
    const metadataRef = db.doc(`${CAMINHO_BASE}/metadata/deputados`);

    await batchManager.set(metadataRef.path, '', { // ID do documento é parte do path para set em doc específico
    ...
    const legislaturaRef = db.doc(`${CAMINHO_LEGISLATURAS}/${legislaturaNumero}`);

    await batchManager.set(legislaturaRef.path, '', { // ID do documento é parte do path
   ...
   */
  private async adicionarOperacoesAoBatchManager(batchManager: FirestoreBatchManager, deputado: DeputadoCompletoTransformado): Promise<void> {
    // Referências para documentos
    const refPerfil = this.getPerfilRef(deputado.codigo);
    const refDadosPessoais = this.getDadosPessoaisRef(deputado.codigo);
    const refUltimoStatus = this.getUltimoStatusRef(deputado.codigo);
  ...
  private getDeputadoLegislaturaRef(codigo: string, legislatura: number): DocumentReference {
    return db.doc(`${CAMINHO_LEGISLATURAS}/${legislatura}/deputados/${codigo}`);
  }
}
