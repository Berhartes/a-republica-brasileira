...
configurarVariaveisAmbiente(); // ESSENCIAL: Configura vars de ambiente ANTES de outros imports
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (resultado.detalhes) { // resultado.detalhes agora é o objeto retornado pelo load do DiscursosDeputadosProcessor
      logger.info(`🎤 Discursos salvos/atualizados: ${resultado.detalhes.discursosSalvos || 0}`);
      logger.info(`👥 Deputados com discursos processados: ${resultado.detalhes.deputadosProcessados || 0}`);
      if (typeof resultado.detalhes.comTranscricao === 'number') {
        logger.info(`📝 Discursos com transcrição: ${resultado.detalhes.comTranscricao}`);
      }
      if (typeof resultado.detalhes.comPalavrasChave === 'number') {
        logger.info(`🏷️ Discursos com palavras-chave: ${resultado.detalhes.comPalavrasChave}`);
      }
      if (resultado.detalhes.batchInfo) {
        logger.info(`📦 Operações de batch: ${resultado.detalhes.batchInfo.sucessos} sucessos, ${resultado.detalhes.batchInfo.falhas} falhas.`);
      }
    }
    logger.info('=====================================');
