...
import { db } from '../utils/storage/firestore/config'; // Admin SDK
...
import { endpoints } from '../config/endpoints'; // Adicionado
import * as api from '../utils/api'; // Adicionado
...
// Interface para discurso completo transformado
...
// Interface para resultado da extração de discursos
interface DiscursosExtractionResult {
  deputadoId: string;
  totalDiscursos: number;
  totalPaginas: number;
  discursos: DiscursoTransformado[];
  anosComDiscursos: number[];
  periodosProcessados?: Array<{ ano: number; mes: number }>; // Para modo atualização
  erro?: string;
}
...
class DiscursosExtractor {
   ...
   */
  async extractDiscursosAtualizacao(deputadoId: string | number, legislatura: number): Promise<DiscursosExtractionResult> {
    try {
      ...
      const discursosNovos = await this.filtrarDiscursosNovos(
        discursosRecentes, 
        dadosExistentes.discursosExistentes
      );
   ...
   */
  private async verificarDadosExistentes(deputadoId: string): Promise<{ discursosExistentes: string[] }> {
    try {
      ...
      const discursosExistentes = (dados?.items || []).map((item: any) => item.id);
   ...
   * @returns Data limite
   */
  private calcularDataLimite60Dias(): Date {
   ...
   */
  private async filtrarDiscursosNovos(
    discursosRecentes: DiscursoTransformado[], 
    idsExistentes: string[]
  ): Promise<DiscursoTransformado[]> {
   ...
   */
  private extrairPeriodosDiscursos(discursos: DiscursoTransformado[]): Array<{ ano: number; mes: number }> {
   ...
   */
  async extractDiscursosCompletos(deputadoId: string | number, legislatura: number): Promise<DiscursosExtractionResult> {
    try {
      logger.info(`Iniciando extração completa de discursos do deputado ${deputadoId}`);
      
      const todosDiscursos: DiscursoTransformado[] = [];
      let paginaAtual = 1;
      let totalPaginas = 1;
      ...
      while (temMaisPaginas) {
        try {
          ...
          const response = await withRetry(
            ...
            `Extração discursos do deputado ${deputadoId} - página ${paginaAtual}`
          );
          ...
          const discursosTransformados = discursosPagina.map((discurso: any) => this.transformDiscurso(discurso, deputadoId.toString()));
   ...
   */
  private transformDiscurso(discurso: any, deputadoId: string): DiscursoTransformado {
    ...
    return {
      ...
      palavrasChave: Array.isArray(discurso.palavrasChave) ? discurso.palavrasChave : 
        (discurso.palavrasChave ? discurso.palavrasChave.split(',').map((p: string) => p.trim()) : []),
...
async function processarDiscursosDeputados() {
  try {
    ...
    const idsDeputados = deputadosParaProcessar.map((deputado: any) => deputado.id);
    ...
    for (let i = 0; i < idsDeputados.length; i += concorrencia) {
      ...
      const promessas = lote.map(async (idDeputado: string) => {
        try {
          let resultadoDiscursos: DiscursosExtractionResult;
 ...
 */
async function salvarDiscursosFirestore(resultadoDiscursos: DiscursosExtractionResult, modoAtualizacao = false): Promise<void> {
  try {
    ...
    let dadosFinais: any;
    let discursosFinais: DiscursoTransformado[];
    ...
    if (modoAtualizacao && totalDiscursos > 0) {
      ...
      const discursosExistentes: DiscursoTransformado[] = docDiscursos.exists ? (docDiscursos.data()?.items || []) : [];
      ...
      const anosFinais = [...new Set(discursosFinais.map((d: DiscursoTransformado) => d.anoDiscurso))].sort();
      
      // 4. Atualizar controle de períodos processados
      const periodosProcessadosExistentes = (dadosExistentes as any)?.periodosProcessados || [];
      ...
      if (periodosProcessados && periodosProcessados.length > 0) {
        ...
        periodosProcessados.forEach(({ ano, mes }) => {
          ...
          const jaExiste = periodosProcessadosNovos.some((p: any) => p.periodo === chave);
          if (!jaExiste) {
      ...
      dadosFinais = {
        ...
        estatisticasPorAno: anosFinais.map((ano: number) => {
          const discursosAno = discursosFinais.filter((d: DiscursoTransformado) => d.anoDiscurso === ano);
        ...
        estatisticasPorTipo: Object.entries(
          discursosFinais.reduce((acc: Record<string, number>, discurso: DiscursoTransformado) => {
      ...
      const periodosProcessadosCompleto: Array<{ periodo: string; dataProcessamento: string }> = [];
      const agora = new Date().toISOString();
      ...
      dadosFinais = {
        ...
        estatisticasPorAno: anosComDiscursos.map((ano: number) => {
          const discursosAno = discursos.filter((d: DiscursoTransformado) => d.anoDiscurso === ano);
        ...
        estatisticasPorTipo: Object.entries(
          discursos.reduce((acc: Record<string, number>, discurso: DiscursoTransformado) => {
            const tipo = discurso.tipoDiscurso || 'Não informado';
            acc[tipo] = (acc[tipo] || 0) + 1;
            return acc;
          }, {})
