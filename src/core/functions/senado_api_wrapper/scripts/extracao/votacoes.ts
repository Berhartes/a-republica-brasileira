...
import { endpoints } from '../utils/api/endpoints'; // Caminho corrigido
import { withRetry } from '../utils/logging'; // Caminho corrigido (importa do index.ts de logging)
  ...
  async extractVotacoesDetalhadas(codigoSenador: string | number): Promise<PromiseSettledResult<PerfilSenadorResult>> {
    try {
      ...
      const votacoesPromise = withRetry(
        async () => api.get(api.replacePath(endpointConfig.PATH, { codigo: codigoSenador.toString() }), endpointConfig.PARAMS),
        endpoints.REQUEST.RETRY_ATTEMPTS,
        endpoints.REQUEST.RETRY_DELAY,
        ...
        if (!response) {
          ...
          return {
            timestamp: new Date().toISOString(),
            origem: api.replacePath(endpointConfig.PATH, { codigo: codigoSenador.toString() }),
            dados: null,
            metadados: {},
            erro: "Resposta vazia da API"
          };
