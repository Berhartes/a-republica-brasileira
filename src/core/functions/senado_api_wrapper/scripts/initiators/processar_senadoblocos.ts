...
async function main(): Promise<void> {
  ...
  try {
    ...
    const cli = new ETLCommandParser(
      'senado:blocos',
      'Processa blocos parlamentares do Senado Federal'
    );

    // Adicionar opções customizadas específicas deste script
    cli.addCustomOption('--incluir-membros', () => true);
    cli.addCustomOption('--tipo-bloco', (value: string) => value.toUpperCase());
    ...
    // Criar processador
    const processor = new BlocosProcessor(options);
...
`);
  ...
  if (resultado.detalhes) {
    ...
    const { totalBlocos, totalMembros } = resultado.detalhes;
    
    if (totalBlocos !== undefined) {
      console.log(`   • Blocos processados: ${totalBlocos}`);
    }
    if (totalMembros !== undefined) {
      console.log(`   • Membros processados: ${totalMembros}`);
    }
  }
}
...
export { main as processarBlocos };
export default main;
