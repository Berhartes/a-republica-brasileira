...
async function processarSenadorMateria(legislaturaEspecifica?: number): Promise<void> {
  try {
    ...
    const cli = new ETLCommandParser('senado:materias', 'Processa matérias legislativas de senadores');
    const opcoes = cli.parse();
    ...
    // 1.5 Dados extraídos (sem exportação)
    logger.info('1.5 Etapa de extração concluída');
    ...
    // 2.2 Transformação concluída (sem exportação)
    logger.info('2.2 Etapa de transformação concluída');
...
if (require.main === module) {
  ...
  const legislaturaEspecifica = opcoes.legislatura;
