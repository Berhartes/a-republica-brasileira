/**
 * Script para processar discursos de senadores
 * 
 * Este script utiliza o sistema modular ETL para extrair,
 * transformar e carregar discursos de senadores.
 * 
 * Uso:
 *   npm run senado:discursos -- [opções]
 * 
 * Exemplos:
 *   npm run senado:discursos                          # Processa discursos do último ano
 *   npm run senado:discursos -- --senador 123         # Discursos de um senador específico
 *   npm run senado:discursos -- --data-inicio 2024-01-01 --data-fim 2024-12-31
 *   npm run senado:discursos -- --pc --limite 100     # Salva no PC, limitado a 100
 * 
 * Para mais opções, use: npm run senado:discursos -- --help
 */
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const cli = new ETLCommandParser(
      'senado:discursos',
      'Processa discursos de senadores do Senado Federal'
    );

    // Adicionar opções customizadas específicas deste script
    cli.addCustomOption('--tipo', (value: string) => value.toUpperCase());
    cli.addCustomOption('--palavras-chave', (value: string) => value.split(','));
    ...
    logger.info('Iniciando processamento de discursos...');
...
╔═══════════════════════════════════════════════════════════════╗
║                                                               ║
║        💬  PROCESSADOR DE DISCURSOS DE SENADORES  💬          ║
║                                                               ║
...
export { main as processarDiscursos };
export default main;
