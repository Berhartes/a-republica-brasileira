...
function processarLegislaturaEspecifica(): void {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--exportar' || arg === '-e') {
      // A variável exportarDados foi removida pois não era utilizada.
      // Se a funcionalidade de exportação for necessária, ela precisará ser reimplementada.
      logger.info('Opção --exportar detectada, mas a funcionalidade de exportação precisa ser implementada.');
