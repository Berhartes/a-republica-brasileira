  ...
  parse(): ETLOptions {
    ...
    for (let i = 0; i < this.args.length; i++) {
      ...
      // Opções customizadas
        ...
        const key = arg.replace('--', '') as keyof ETLOptions; // Corrigido com asserção de tipo
        if (parser && nextArg) {
          // Se há um parser, o tipo de options[key] será determinado pelo retorno do parser.
          // A asserção de tipo para 'key' já ajuda o TS a entender que 'key' é uma chave válida.
          // O parser deve retornar um valor compatível com o tipo de options[key].
          (options as any)[key] = parser(nextArg);
          i++;
        } else {
          // Se não há parser, assume-se que é uma flag booleana.
          // Isso requer que 'key' corresponda a uma propriedade booleana em ETLOptions.
