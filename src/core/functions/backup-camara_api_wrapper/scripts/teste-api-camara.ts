 ...
 */
async function testarEndpointsFuncionais(): Promise<void> {
 ...
 */
async function testarPerfilDeputado(): Promise<void> {
  ...
  try {
    ...
    if (perfilResponse.status === 200 && perfilResponse.data?.dados) {
      ...
      if (perfil.mandatos && Array.isArray(perfil.mandatos)) {
        ...
        perfil.mandatos.slice(0, 2).forEach((mandato: any, i: number) => {
      ...
      if (perfil.filiacoes && Array.isArray(perfil.filiacoes)) {
        ...
        perfil.filiacoes.slice(0, 2).forEach((filiacao: any, i: number) => {
 ...
 */
async function testarEndpointsInvalidos(): Promise<void> {
 ...
 */
async function testarDadosComplementares(): Promise<void> {
