...
export class ETLCommandParser {
  ...
  parse(): ETLOptions {
    ...
    const options: Partial<ETLOptions> = {
      destino: [] // Inicializar destino como um array vazio
    };
    ...
    for (let i = 0; i < this.args.length; i++) {
      ...
      // Deputado específico
      else if (arg === '--deputado' || arg === '-d') { // Alterado de senador para deputado
        options.deputado = nextArg; // Alterado de senador para deputado
        i++;
      }
      
      // Destino
      else if (arg === '--firestore') {
        if (!options.destino?.includes('firestore')) options.destino?.push('firestore');
      }
      else if (arg === '--emulator') {
        if (!options.destino?.includes('emulator')) options.destino?.push('emulator');
      }
      else if (arg === '--pc' || arg === '--local') {
        if (!options.destino?.includes('pc')) options.destino?.push('pc');
      }
      ...
      // Opções customizadas
        ...
        if (typeof config === 'function') {
          ...
          const optionName = arg.replace('--', ''); // Obter o nome da opção sem os hífens
          let value: any;

          // Verificar se há um próximo argumento e se ele não é outra flag
          if (nextArg && !nextArg.startsWith('-')) {
            value = nextArg;
            i++; // Consumir o próximo argumento
          } else {
            // Se não há próximo argumento ou o próximo é outra flag,
            // e não há um valor padrão definido, a flag é booleana e deve ser true
            if (config.defaultValue === undefined) {
              value = true; // Tratar como flag booleana (presente = true)
            } else {
              value = config.defaultValue; // Usar o valor padrão se definido
            }
          }
          ...
          // Se não há config para a opção, apenas define como true (comportamento padrão para flags desconhecidas)
          options[optionName] = true;
        }
      }
    ...
    const finalOptions: ETLOptions = {
      ...options,
      destino: options.destino && options.destino.length > 0 ? options.destino : ['firestore'],
    } as ETLOptions; // Cast para ETLOptions para satisfazer o compilador após a remoção de Partial
...
ARGUMENTOS:
  [legislatura]              Número da legislatura (${etlConfig.camara.legislatura.min}-${etlConfig.camara.legislatura.max})
                            Se não informado, usa a legislatura atual da configuração.
...
FILTROS:
  ...
  --deputado, -d <código>   Processa apenas o deputado especificado
...
VARIÁVEIS DE AMBIENTE:
  ...
  CAMARA_TIMEOUT            Timeout em ms (padrão: ${etlConfig.camara.timeout})
  CAMARA_ITEMS_PER_PAGE     Itens por página em requisições (padrão: ${etlConfig.camara.itemsPerPage})
...
`);
  ...
  private parseLegislaturaShortcut(arg: string): number {
    ...
    if (num < etlConfig.camara.legislatura.min || num > etlConfig.camara.legislatura.max) {
      throw new Error(`Legislatura inválida: ${num}. Deve estar entre ${etlConfig.camara.legislatura.min} e ${etlConfig.camara.legislatura.max}`);
    }
    return num;
  }
   ...
   */
  // Removida a função validateDestino pois agora múltiplos destinos são permitidos
