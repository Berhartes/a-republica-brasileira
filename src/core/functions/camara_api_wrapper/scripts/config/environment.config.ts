...
export function configurarVariaveisAmbiente(): void {
  ...
  const hasPC = args.includes('--pc') || args.includes('--local'); // Adicionado --local como alias

  // Não validar mais exclusividade aqui, ETLCommandParser agora retorna um array de destinos.
  // A lógica abaixo definirá as variáveis de ambiente com base nas flags presentes.
  // Múltiplas podem ser verdadeiras.

  // Configurar baseado nas flags
  // Resetar flags antes de configurar para evitar contaminação de execuções anteriores no mesmo processo (improvável para CLI, mas boa prática)
  process.env.USE_FIRESTORE_EMULATOR = 'false';
  process.env.USE_REAL_FIRESTORE = 'false';
  process.env.SAVE_TO_PC = 'false';
  
  if (hasEmulator) {
    console.log('🔌 Configurando para usar Firestore Emulator');
    process.env.USE_FIRESTORE_EMULATOR = 'true';
    process.env.USE_FIRESTORE_EMULATOR = 'true';
    // Não definir USE_REAL_FIRESTORE como false aqui, pois pode ser usado em conjunto
  ...
  if (hasFirestore) {
    ...
    // Não definir USE_FIRESTORE_EMULATOR como false aqui, pois pode ser usado em conjunto (embora --emulator deva ter precedência se ambos)
    // Se hasEmulator também for true, USE_FIRESTORE_EMULATOR já estará 'true'.
    // A lógica em createBatchManager dará precedência ao emulador se ambos estiverem 'true'.
  }
  ...
  if (hasPC) {
    ...
    process.env.PC_SAVE_DIR = process.env.PC_SAVE_DIR || 'C:\\Users\\Kast Berhartes\\projetos-web-berhartes\\a-republica-brasileira\\src\\core\\output_pc'; // Adicionado um subdiretório output_pc
  }

  // Se nenhuma flag de destino específica (--firestore, --emulator, --pc) foi passada,
  // aplicar o padrão (Firestore Real).
  // Isso é importante porque ETLCommandParser agora define options.destino como ['firestore'] por padrão.
  // Esta função (configurarVariaveisAmbiente) é chamada ANTES do parse completo de ETLCommandParser,
  // então ela ainda depende de process.argv.
  // O ideal seria refatorar para que configurarVariaveisAmbiente receba as options parseadas.
  // Por ora, esta lógica de fallback garante que algo seja definido se nenhuma flag for passada.
  if (!hasEmulator && !hasFirestore && !hasPC) {
    if (process.env.NODE_ENV === 'production') {
      console.log('☁️ Nenhuma flag de destino. Ambiente de produção detectado - usando Firestore Real por padrão.');
      process.env.USE_REAL_FIRESTORE = 'true';
    } else {
      console.log('🏗️ Nenhuma flag de destino. Ambiente de desenvolvimento - usando Firestore Real por padrão.');
      console.log('   Use --emulator para usar o emulador e/ou --pc para salvar localmente.');
      process.env.USE_REAL_FIRESTORE = 'true';
    }
  }
