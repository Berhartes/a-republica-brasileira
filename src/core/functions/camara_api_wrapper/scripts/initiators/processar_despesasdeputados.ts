...
import { parseArgs, CommandOptions } from '../utils/cli/args-parser';
...
const ITENS_POR_PAGINA = 100; // Máximo de itens por página na API
...
// Interface para despesa completa transformada
...
// Interface para resultado da extração de despesas
interface DespesasExtractionResult {
  deputadoId: string;
  totalDespesas: number;
  totalPaginas: number;
  despesas: DespesaTransformada[];
  anosComDespesas: number[];
  valorTotal: number;
  mesesProcessados?: Array<{ ano: number; mes: number }>; // Para modo atualização
  erro?: string;
}
 ...
 */
function calcularTamanhoBytes(obj: any): number {
  try {
 ...
 */
function dividirEmBatchesSeguras(despesas: DespesaTransformada[]): DespesaTransformada[][] {
  const batches: DespesaTransformada[][] = [];
  let batchAtual: DespesaTransformada[] = [];
  let tamanhoAtual = 0;
...
class DespesasExtractor {
   ...
   */
  async extractDespesasAtualizacao(deputadoId: string | number, legislatura: number): Promise<DespesasExtractionResult> {
    try {
      ...
      const todasDespesasNovas: DespesaTransformada[] = [];
      let totalPaginas = 0;
   ...
   */
  private async verificarDadosExistentes(deputadoId: string): Promise<{ mesesProcessados: Record<string, string> }> {
    try {
      const deputadoRef = getDb().doc(`congressoNacional/camaraDeputados/despesas/${deputadoId}`);
   ...
   */
  private calcularMesesFaltantes(mesesProcessados: Record<string, string>): Array<{ ano: number; mes: number }> {
    const agora = new Date();
    const mesesParaVerificar: Array<{ ano: number; mes: number }> = [];
   ...
   */
  private async extractDespesasPorMes(deputadoId: string | number, ano: number, mes: number, legislatura: number): Promise<{ despesas: DespesaTransformada[]; totalPaginas: number }> {
    const despesasMes: DespesaTransformada[] = [];
    let paginaAtual = 1;
    ...
    while (temMaisPaginas) {
      try {
        ...
        const response = await withRetry(
          ...
          `Extração ${ano}-${mes} do deputado ${deputadoId} - página ${paginaAtual}`
        );
        ...
        const despesasTransformadas = despesasPagina.map((despesa: any) => this.transformDespesa(despesa, deputadoId.toString()));
   ...
   */
  async extractDespesasCompletas(deputadoId: string | number, legislatura: number): Promise<DespesasExtractionResult> {
    try {
      logger.info(`Iniciando extração completa de despesas do deputado ${deputadoId}`);
      
      const todasDespesas: DespesaTransformada[] = [];
      let paginaAtual = 1;
      let totalPaginas = 1;
      let temMaisPaginas = true;
      ...
      while (temMaisPaginas) {
        try {
          ...
          endpoints.REQUEST.RETRY_ATTEMPTS,
          endpoints.REQUEST.RETRY_DELAY,
          `Extração despesas do deputado ${deputadoId} - página ${paginaAtual}`
          );
   ...
   */
  private transformDespesa(despesa: any, deputadoId: string): DespesaTransformada {
    return {
      // Dados básicos
 ...
 */
async function carregarTodasDespesasExistentes(deputadoId: string): Promise<DespesaTransformada[]> {
  try {
    const despesasExistentes: DespesaTransformada[] = [];
    
    // Buscar documento único primeiro (compatibilidade com versão anterior)
    const docUnicoRef = getDb().doc(`congressoNacional/camaraDeputados/despesas/${deputadoId}/dados/items`);
    ...
    for (let i = 0; i < 20; i++) {
      const batchRef = getDb().doc(`congressoNacional/camaraDeputados/despesas/${deputadoId}/dados/batch_${i}`);
 ...
 */
function calcularEstatisticasPorAno(despesas: DespesaTransformada[]) {
 ...
 */
function calcularEstatisticasPorTipo(despesas: DespesaTransformada[]) {
  const contadorTipos = despesas.reduce((acc: Record<string, number>, despesa) => {
 ...
 */
async function salvarComBatches(deputadoId: string, despesas: DespesaTransformada[], dadosFinais: any): Promise<void> {
  try {
    ...
    const promessasBatches = batches.map(async (batch, indice) => {
      const batchRef = getDb().doc(`congressoNacional/camaraDeputados/despesas/${deputadoId}/dados/batch_${indice}`);
 ...
 */
async function salvarDocumentoUnico(deputadoId: string, despesas: DespesaTransformada[], dadosFinais: any): Promise<void> {
  try {
    const deputadoRef = getDb().doc(`congressoNacional/camaraDeputados/despesas/${deputadoId}`);
    const despesasRef = getDb().doc(`congressoNacional/camaraDeputados/despesas/${deputadoId}/dados/items`);
...
/**
 * Limpa documentos existentes (documentos anuais, batches antigos e documento único)
 */
async function limparDocumentosExistentes(deputadoId: string): Promise<void> {
  try {
    const promessasLimpeza: Promise<any>[] = [];
    
    // Limpar documento único (estrutura antiga) se existir
    const docUnicoRef = getDb().doc(`congressoNacional/camaraDeputados/despesas/${deputadoId}/dados/items`);
    promessasLimpeza.push(docUnicoRef.delete().catch(() => {})); // Ignorar erros se não existir
    
    // Limpar batches existentes (estrutura antiga) se existirem (buscar até um limite razoável de batches)
    for (let i = 0; i < 20; i++) { // Assumindo um máximo de 20 batches
      const batchRef = getDb().doc(`congressoNacional/camaraDeputados/despesas/${deputadoId}/dados/batch_${i}`);
      promessasLimpeza.push(batchRef.delete().catch(() => {})); // Ignorar erros se não existir
    }

    // Limpar documentos anuais (nova estrutura) se existirem
    // Para isso, precisamos listar os documentos na subcoleção 'dados'
    const dadosCollectionRef = getDb().collection(`congressoNacional/camaraDeputados/despesas/${deputadoId}/dados`);
    const snapshot = await dadosCollectionRef.get();
    snapshot.docs.forEach(doc => {
      promessasLimpeza.push(doc.ref.delete().catch(() => {})); // Ignorar erros
    });
...
Exemplos:
  ...
  npx ts-node -P ts-node -P tsconfig.scripts.json scripts/initiators/processar_despesasdeputados.ts --concorrencia 3
`);
  process.exit(0);
}
...
async function processarDespesasDeputados() {
  try {
    ...
    logger.info(`Configurações: limite=${limite}, concorrência=${concorrencia}, modo=${modoAtualizacao ? 'ATUALIZAÇÃO INCREMENTAL (60 dias)' : 'COMPLETO'}`);    logger.info(`${modoAtualizacao ? '📋 Modo Atualização: Processará apenas meses faltantes dos últimos 60 dias' : '📋 Modo Completo: Processará todas as despesas disponíveis'}`);
    ...
    for (let i = 0; i < idsDeputados.length; i += concorrencia) {
      ...
      const promessas = lote.map(async (idDeputado) => {
        try {
          let resultadoDespesas: DespesasExtractionResult;
 ...
 */
async function salvarDespesasFirestore(resultadoDespesas: DespesasExtractionResult, modoAtualizacao = false): Promise<void> {
  try {
    ...
    const deputadoDespesasRef = getDb().doc(`congressoNacional/camaraDeputados/despesas/${deputadoId}`);

    let dadosFinais: any;
    let despesasFinais: DespesaTransformada[];
    ...
    if (modoAtualizacao && totalDespesas > 0) {
      ...
      logger.info(`Dados existentes: ${despesasExistentes.length} despesas, valor total: R$ ${(dadosExistentes as any)?.valorTotal || 0}`);
      ...
      const anosFinais = [...new Set(despesasFinais.map((d: DespesaTransformada) => d.ano))].sort();
      const valorTotalFinal = despesasFinais.reduce((total: number, despesa: DespesaTransformada) => total + despesa.valorLiquido, 0);
      
      // 5. Atualizar controle de meses processados
      const mesesProcessadosExistentes = (dadosExistentes as any)?.mesesProcessados || {};
      ...
      dadosFinais = {
        ...
        totalPaginas: ((dadosExistentes as any)?.totalPaginas || 0) + totalPaginas,
      ...
      const mesesProcessadosCompleto: Record<string, string> = {};
      const agora = new Date().toISOString();
    ...
    // 1. Limpar documentos existentes (tanto único quanto batches antigos e documentos anuais)
    await limparDocumentosExistentes(deputadoId);

    // 2. Agrupar despesas por ano
    const despesasPorAno: Record<number, DespesaTransformada[]> = despesasFinais.reduce((acc: Record<number, DespesaTransformada[]>, despesa) => {
      const ano = despesa.ano;
      if (!acc[ano]) {
        acc[ano] = [];
      }
      acc[ano].push(despesa);
      return acc;
    }, {} as Record<number, DespesaTransformada[]>); // Adicionar tipagem explícita para o acumulador

    // 3. Salvar cada ano como um documento separado na subcoleção 'dados'
    const promessasSalvamentoAnual: Promise<any>[] = [];
    for (const ano of Object.keys(despesasPorAno).map(Number).sort()) {
      const despesasDoAno = despesasPorAno[ano];
      const anoDocRef = getDb().doc(`congressoNacional/camaraDeputados/despesas/${deputadoId}/dados/${ano}`);
      promessasSalvamentoAnual.push(anoDocRef.set({
        ano: ano,
        totalDespesas: despesasDoAno.length,
        ultimaAtualizacao: new Date().toISOString(),
        items: despesasDoAno
      }));
      logger.info(`Salvando ${despesasDoAno.length} despesas para o ano ${ano} do deputado ${deputadoId}`);
    }

    // 4. Salvar o documento principal do deputado com os dados finais
    const deputadoRef = getDb().doc(`congressoNacional/camaraDeputados/despesas/${deputadoId}`);
    const dadosPrincipais = {
      ...dadosFinais,
      estruturaArmazenamento: 'por_ano', // Nova estrutura
      totalAnosComDespesas: Object.keys(despesasPorAno).length,
      ultimaAtualizacao: new Date().toISOString()
    };
    promessasSalvamentoAnual.push(deputadoRef.set(dadosPrincipais, { merge: true }));

    // 5. Executar todas as operações de salvamento
    await Promise.all(promessasSalvamentoAnual);
