...
import { PerfilDeputadosProcessor } from '../processors/perfil-deputados.processor';
import { logger, LogLevel } from '../utils/logging';
import { ProcessingStatus } from '../types/etl.types';
import { obterNumeroLegislaturaAtual } from '../utils/date/legislatura';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const cli = new ETLCommandParser(
      'camara:perfil',
      'Processa perfis completos de deputados da Câmara dos Deputados'
    );
    ...
    // Definir legislatura padrão se não fornecida
    const legislaturaPadrao = await obterNumeroLegislaturaAtual();
    if (legislaturaPadrao === null) {
      throw new Error('Não foi possível determinar a legislatura atual. Por favor, especifique-a usando --legislatura <número>.');
    }
    options.legislatura = options.legislatura || legislaturaPadrao;
    ...
    logger.info('🔧 Configurações de processamento:');
    logger.info(`   📋 Legislatura: ${options.legislatura || 'atual'}`);
    logger.info(`   🎯 Limite: ${options.limite || 'sem limite'}`);
    logger.info(`   💾 Destino: ${options.destino}`);

    if (options.deputado) {
      logger.info(`   👤 Deputado específico: ${options.deputado}`);
    }

    if (options.partido) {
      logger.info(`   🏛️ Partido: ${options.partido}`);
    }

    if (options.uf) {
      logger.info(`   📍 UF: ${options.uf}`);
    }

    // Exibir opções de extração
    logger.info('🔧 Opções de extração:');
    logger.info(`   📋 Mandatos: ${options.mandatos ? '✅ SIM' : '❌ NÃO'}`);
    logger.info(`   🏛️ Filiações: ${options.filiacoes ? '✅ SIM' : '❌ NÃO'}`);
    logger.info(`   📷 Fotos: ${options.fotos ? '✅ SIM' : '❌ NÃO'}`);
    logger.info(`   🏢 Órgãos: ${options.orgaos !== false ? '✅ SIM' : '❌ NÃO'}`);
    logger.info(`   👥 Frentes: ${options.frentes !== false ? '✅ SIM' : '❌ NÃO'}`);

    // Criar processador
    const processor = new PerfilDeputadosProcessor(options);
    ...
    logger.info('Iniciando processamento de perfis de deputados...');
