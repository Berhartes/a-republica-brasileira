...
export class PerfilDeputadosProcessor extends ETLProcessor<ExtractedData, TransformedData> {
  ...
  private transformPerfil(perfilBruto: any): PerfilDeputado {
    const perfil: PerfilDeputado = {
      ...
      uri: perfilBruto.uri,
      nome: perfilBruto.nomeCivil || perfilBruto.ultimoStatus?.nome || '',
      nomeCivil: perfilBruto.nomeCivil,
      nomeEleitoral: perfilBruto.ultimoStatus?.nomeEleitoral,
      ...
      gabinete: perfilBruto.ultimoStatus?.gabinete ? {
        nome: perfilBruto.ultimoStatus.gabinete.nome,
  ...
  async load(data: TransformedData): Promise<any> {
    ...
    const { getPCSaveDirectory } = require('../utils/storage/firestore'); // Para obter o diretório base
    ...
    if (destinos.includes('firestore') || destinos.includes('emulator')) {
      ...
      const batchManager = createBatchManager(); // createBatchManager decidirá se é real ou emulador
