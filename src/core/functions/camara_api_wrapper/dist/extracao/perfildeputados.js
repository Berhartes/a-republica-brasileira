...
export class PerfilDeputadosExtractor {
    ...
    async extractDeputadosLegislatura(legislatura) {
        try {
            ...
            const response = await withRetry(async () => api.get(endpointConfig.PATH, params), endpoints.REQUEST.RETRY_ATTEMPTS, endpoints.REQUEST.RETRY_DELAY, `Extração lista de deputados da legislatura ${legislatura}`);
    ...
    async extractDadosBasicos(codigoDeputado) {
        try {
            ...
            const response = await withRetry(async () => api.get(endpoint, endpointConfig.PARAMS), endpoints.REQUEST.RETRY_ATTEMPTS, endpoints.REQUEST.RETRY_DELAY, `Extração dados básicos do deputado ${codigoDeputado}`);
    ...
    async extractDespesas(codigoDeputado) {
        try {
            ...
            const endpoint = api.replacePath(endpointConfig.PATH, { codigo: codigoDeputado.toString() });
            const response = await withRetry(async () => api.get(endpoint, endpointConfig.PARAMS), endpoints.REQUEST.RETRY_ATTEMPTS, endpoints.REQUEST.RETRY_DELAY, `Extração despesas do deputado ${codigoDeputado}`);
            // Verificação rigorosa da resposta
            if (!response) {
    ...
    async extractDiscursos(codigoDeputado, legislatura) {
        try {
            ...
            try {
                const response = await withRetry(async () => api.get(endpoint, params), endpoints.REQUEST.RETRY_ATTEMPTS, endpoints.REQUEST.RETRY_DELAY, `Extração discursos do deputado ${codigoDeputado}`);
                // Verificação rigorosa da resposta
                if (!response) {
    ...
    async extractEventos(codigoDeputado) {
        try {
            ...
            const endpoint = api.replacePath(endpointConfig.PATH, { codigo: codigoDeputado.toString() });
            const response = await withRetry(async () => api.get(endpoint, endpointConfig.PARAMS), endpoints.REQUEST.RETRY_ATTEMPTS, endpoints.REQUEST.RETRY_DELAY, `Extração eventos do deputado ${codigoDeputado}`);
            // Verificação rigorosa da resposta
            if (!response) {
    ...
    async extractOrgaos(codigoDeputado) {
        try {
            ...
            const endpoint = api.replacePath(endpointConfig.PATH, { codigo: codigoDeputado.toString() });
            const response = await withRetry(async () => api.get(endpoint, endpointConfig.PARAMS), endpoints.REQUEST.RETRY_ATTEMPTS, endpoints.REQUEST.RETRY_DELAY, `Extração órgãos do deputado ${codigoDeputado}`);
            // Verificação rigorosa da resposta
            if (!response) {
    ...
    async extractFrentes(codigoDeputado) {
        try {
            ...
            const endpoint = api.replacePath(endpointConfig.PATH, { codigo: codigoDeputado.toString() });
            const response = await withRetry(async () => api.get(endpoint, endpointConfig.PARAMS), endpoints.REQUEST.RETRY_ATTEMPTS, endpoints.REQUEST.RETRY_DELAY, `Extração frentes do deputado ${codigoDeputado}`);
            // Verificação rigorosa da resposta
            if (!response) {
    ...
    async extractOcupacoes(codigoDeputado) {
        try {
            ...
            const endpoint = api.replacePath(endpointConfig.PATH, { codigo: codigoDeputado.toString() });
            const response = await withRetry(async () => api.get(endpoint, endpointConfig.PARAMS), endpoints.REQUEST.RETRY_ATTEMPTS, endpoints.REQUEST.RETRY_DELAY, `Extração ocupações do deputado ${codigoDeputado}`);
            // Verificação rigorosa da resposta
            if (!response) {
    ...
    async extractMandatosExternos(codigoDeputado) {
        try {
            ...
            const endpoint = api.replacePath(endpointConfig.PATH, { codigo: codigoDeputado.toString() });
            const response = await withRetry(async () => api.get(endpoint, endpointConfig.PARAMS), endpoints.REQUEST.RETRY_ATTEMPTS, endpoints.REQUEST.RETRY_DELAY, `Extração mandatos externos do deputado ${codigoDeputado}`);
            // Verificação rigorosa da resposta
            if (!response) {
    ...
    async extractHistorico(codigoDeputado) {
        try {
            ...
            const endpoint = api.replacePath(endpointConfig.PATH, { codigo: codigoDeputado.toString() });
            const response = await withRetry(async () => api.get(endpoint, endpointConfig.PARAMS), endpoints.REQUEST.RETRY_ATTEMPTS, endpoints.REQUEST.RETRY_DELAY, `Extração histórico do deputado ${codigoDeputado}`);
            // Verificação rigorosa da resposta
            if (!response) {
    ...
    async extractProfissoes(codigoDeputado) {
        try {
            ...
            const endpoint = api.replacePath(endpointConfig.PATH, { codigo: codigoDeputado.toString() });
            const response = await withRetry(async () => api.get(endpoint, endpointConfig.PARAMS), endpoints.REQUEST.RETRY_ATTEMPTS, endpoints.REQUEST.RETRY_DELAY, `Extração profissões do deputado ${codigoDeputado}`);
            // Verificação rigorosa da resposta
            if (!response) {
