...
async function main() {
    ...
    try {
        ...
        const processorOptions = {
            ...
            concorrencia: options.concorrencia,
        ...
        if (processorOptions.dataInicio)
            logger.info(`📅 Data início: ${processorOptions.dataInicio}`);
        if (processorOptions.dataFim)
            logger.info(`📅 Data fim: ${processorOptions.dataFim}`);
        if (processorOptions.tipo)
            logger.info(`🎯 Tipo: ${processorOptions.tipo}`);
        if (palavrasChave.length > 0)
            logger.info(`🔍 Palavras-chave: ${palavrasChave.join(', ')}`);
        if (processorOptions.limite)
            logger.info(`🔢 Limite: ${processorOptions.limite} deputados`);
