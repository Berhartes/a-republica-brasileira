 ...
 */
export async function obterLegislaturaAtual(data) {
    ...
    try {
        ...
        const endpoint = api.replacePath(endpoints.LEGISLATURAS.LISTA.PATH, { data });
        // Fazer a requisição usando o utilitário de API
        const response = await withRetry(async () => api.get(endpoint, endpoints.LEGISLATURAS.LISTA.PARAMS), endpoints.REQUEST.RETRY_ATTEMPTS, endpoints.REQUEST.RETRY_DELAY, `Obter legislatura ${data}`);
 ...
 */
export async function obterPeriodoLegislatura(numeroLegislatura) {
    try {
 ...
 */
export async function obterNumeroLegislaturaAtual(data) {
