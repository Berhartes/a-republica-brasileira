...
import { exportarParaJSON, exportarParaCSV, calcularCompletude, verificarConsistencia, gerarEstatisticasGerais, criarDadosResumidos, exportarObjeto, criarEstruturaDiretorios } from './exportacao-avanc';
 ...
 */
export function exportToJson(dados, filePath) {
    try {
 ...
 */
export function exportItemsAsIndividualFiles(dados, baseDir, getFileName) {
    try {
 ...
 */
export async function exportarDadosFirestore(dados, opcoes, prepararDadosFirestore) {
    try {
 ...
 * @param opcoes - Opções de exportação
 */
export async function exportarDadosBrutos(dados, opcoes) {
    try {
        const timestamp = opcoes.timestamp || new Date().toISOString().replace(/[:.]/g, '-');
        const caminhoBase = opcoes.caminhoBase || 'dados';
 ...
 */
export async function exportarDadosAvancados(dados, opcoes, tempoInicio = Date.now()) {
    try {
        const dataExtracao = new Date().toISOString().split('T')[0];
        const diretorioBase = await criarEstruturaDiretorios(0, // Não usamos legislatura aqui, é um valor genérico
        ...
        if (opcoes.formato === 'json' || opcoes.formato === 'ambos') {
            if (opcoes.nivelDetalhamento === 'completo' || opcoes.nivelDetalhamento === 'ambos') {
                await exportarParaJSON(dados, path.join(diretorioBase, 'dados', 'dados_completos.json'), opcoes.comprimir);
            }
            if (opcoes.nivelDetalhamento === 'resumido' || opcoes.nivelDetalhamento === 'ambos') {
                await exportarParaJSON(dadosResumidos, path.join(diretorioBase, 'dados', 'dados_resumidos.json'), opcoes.comprimir);
            }
        }
        if (opcoes.formato === 'csv' || opcoes.formato === 'ambos') {
            if (opcoes.nivelDetalhamento === 'completo' || opcoes.nivelDetalhamento === 'ambos') {
                await exportarParaCSV(dados, path.join(diretorioBase, 'dados', 'dados_completos.csv'), opcoes.comprimir);
            }
            if (opcoes.nivelDetalhamento === 'resumido' || opcoes.nivelDetalhamento === 'ambos') {
                await exportarParaCSV(dadosResumidos, path.join(diretorioBase, 'dados', 'dados_resumidos.csv'), opcoes.comprimir);
            }
        }
