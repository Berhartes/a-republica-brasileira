 ...
 */
export async function criarEstruturaDiretorios(legislatura, dataExtracao = new Date().toISOString().split('T')[0], caminhoBase = 'dados') {
 ...
 */
export async function exportarParaJSON(dados, caminhoArquivo, comprimir = false) {
 ...
 * @returns Caminho do arquivo gerado
 */
export async function exportarParaCSV(dados, caminhoArquivo, comprimir = false) {
 ...
 */
export function calcularCompletude(dados) {
    if (!dados.length) {
 ...
 */
export function verificarConsistencia(dados) {
    if (!dados.length) {
        return {
            tiposValidos: {},
            dataCoerente: false,
 ...
 */
export function gerarEstatisticasGerais(dados) {
 ...
 */
export function criarDadosResumidos(dados) {
 ...
 * @returns Caminho do arquivo gerado
 */
export async function exportarObjeto(objeto, caminhoArquivo, comprimir = false) {
 ...
 */
export async function exportarDados(dados, legislatura, opcoes, tempoInicio) {
    try {
