...
import { ProcessingStatus // Importar ProcessingStatus
 ...
 */
export class PerfilDeputadosProcessor extends ETLProcessor {
    constructor(options) {
        super(options);
    ...
    async extractDeputadoEspecifico(deputadoId, legislatura) {
        try {
            ...
            const response = await withRetry(() => api.get(endpoint, endpointConfig.PARAMS), etlConfig.camara.maxRetries, etlConfig.camara.pauseBetweenRequests, `Perfil do deputado ${deputadoId}`);
    ...
    async extractDeputadosLegislatura(legislatura) {
        try {
            ...
            do {
                ...
                const response = await withRetry(() => api.get(endpointConfig.PATH, params), etlConfig.camara.maxRetries, etlConfig.camara.pauseBetweenRequests, `Lista de deputados da legislatura ${legislatura}, página ${pagina}`);
    ...
    async extractPerfilCompleto(deputadoId) {
        const endpointConfig = endpoints.DEPUTADOS.PERFIL;
        const endpoint = api.replacePath(endpointConfig.PATH, { codigo: deputadoId });
        const response = await withRetry(() => api.get(endpoint, endpointConfig.PARAMS), etlConfig.camara.maxRetries, etlConfig.camara.pauseBetweenRequests, `Perfil completo do deputado ${deputadoId}`);
        if (!response || !response.dados) {
    ...
    async extractOrgaos(deputadoId) {
        ...
        const endpoint = api.replacePath(endpointConfig.PATH, { codigo: deputadoId });
        const response = await withRetry(() => api.get(endpoint, endpointConfig.PARAMS), etlConfig.camara.maxRetries, etlConfig.camara.pauseBetweenRequests, `Órgãos do deputado ${deputadoId}`);
    ...
    async extractFrentes(deputadoId) {
        ...
        const endpoint = api.replacePath(endpointConfig.PATH, { codigo: deputadoId });
        const response = await withRetry(() => api.get(endpoint, endpointConfig.PARAMS), etlConfig.camara.maxRetries, etlConfig.camara.pauseBetweenRequests, `Frentes do deputado ${deputadoId}`);
        return response?.dados || [];
    }
    /**
    ...
    async extractOcupacoes(deputadoId) {
        ...
        const endpoint = api.replacePath(endpointConfig.PATH, { codigo: deputadoId });
        const response = await withRetry(() => api.get(endpoint, endpointConfig.PARAMS), etlConfig.camara.maxRetries, etlConfig.camara.pauseBetweenRequests, `Ocupações do deputado ${deputadoId}`);
        return response?.dados || [];
    }
    /**
    ...
    async extractMandatosExternos(deputadoId) {
        ...
        const endpoint = api.replacePath(endpointConfig.PATH, { codigo: deputadoId });
        const response = await withRetry(() => api.get(endpoint, endpointConfig.PARAMS), etlConfig.camara.maxRetries, etlConfig.camara.pauseBetweenRequests, `Mandatos externos do deputado ${deputadoId}`);
        return response?.dados || [];
    }
    /**
    ...
    async extractHistorico(deputadoId) {
        ...
        const endpoint = api.replacePath(endpointConfig.PATH, { codigo: deputadoId });
        const response = await withRetry(() => api.get(endpoint, endpointConfig.PARAMS), etlConfig.camara.maxRetries, etlConfig.camara.pauseBetweenRequests, `Histórico do deputado ${deputadoId}`);
        return response?.dados || [];
    }
    /**
    ...
    async extractProfissoes(deputadoId) {
        ...
        const endpoint = api.replacePath(endpointConfig.PATH, { codigo: deputadoId });
        const response = await withRetry(() => api.get(endpoint, endpointConfig.PARAMS), etlConfig.camara.maxRetries, etlConfig.camara.pauseBetweenRequests, `Profissões do deputado ${deputadoId}`);
        return response?.dados || [];
    }
    /**
