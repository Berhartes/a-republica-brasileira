...
import { firestoreDb as db } from '../utils/storage/firestore'; // Importar firestoreDb
 ...
 */
export class DiscursosDeputadosProcessor extends ETLProcessor {
    constructor(options) {
        super(options);
        this.storageManager = createBatchManager();
    }
    /**
     * Nome do processador
     */
    ...
    async extractDeputadosLegislatura(legislatura) {
        try {
            ...
            const response = await withRetry(() => get(endpointConfig.PATH, params), etlConfig.camara.maxRetries, etlConfig.camara.pauseBetweenRequests, `Lista de deputados da legislatura ${legislatura}`);
            ...
            const deputados = response.dados.map((dep) => ({
                id: dep.id?.toString() || '',
                nome: dep.nome || '',
    ...
    async extractDiscursosCompletos(deputadoId) {
        ...
        try {
            ...
            const todosDiscursos = await apiClient.getAllPages(endpoint, baseParams, {
    ...
    async load(data) {
        ...
        try {
            // Usar o createBatchManager para grandes volumes
            ...
            for (const [deputadoId, discursos] of Object.entries(discursosPorDeputado)) {
                ...
                for (const doc of discursosDocuments) {
                    // Usar a instância do Firestore diretamente para subcoleções
                    await db.collection('discursos').doc(deputadoId).collection('dados').doc(doc.id).set(doc.data);
                }
            }
            ...
            await multiBatchManager.set('metadados', `discursos_legislatura_${this.context.options.legislatura}`, metadata);
            // 3. Executar todos os batches
            this.emitProgress(ProcessingStatus.CARREGANDO, 90, 'Executando commit dos batches');
            const batchResult = await multiBatchManager.commit(); // Alterado para commit()
            results.push(batchResult);
            // Resultado consolidado
            const finalResult = {
                total: results.reduce((sum, r) => sum + r.total, 0), // Adicionado
                processados: results.reduce((sum, r) => sum + r.processados, 0), // Adicionado
                sucessos: results.reduce((sum, r) => sum + r.sucessos, 0),
                falhas: results.reduce((sum, r) => sum + r.falhas, 0),
                tempoOperacao: results.reduce((sum, r) => sum + (r.tempoOperacao || 0), 0),
                detalhes: {
