...
exports.exibirAjuda = exibirAjuda;
/**
 ...
 */
function detectarArgumentoLegislatura(arg) {
...
function parseArgs() {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (legislaturaDetectada !== null) {
            ...
            logging_1.logger.info(`Legislatura detectada via argumento numérico: ${legislaturaDetectada}`);
            continue;
        }
        if (arg.startsWith('--')) {
            ...
            if (legislaturaArg > 0 && legislaturaArg <= 100) {
                ...
                logging_1.logger.info(`Legislatura especificada via primeiro argumento: ${legislaturaArg}`);
            }
            else {
                logging_1.logger.error(`Legislatura inválida: ${arg}. Deve ser um número entre 1 e 100.`);
                process.exit(1);
            }
        }
    }
    // Log das opções encontradas
    logging_1.logger.info('Opções da linha de comando:', options);
 ...
 */
function getBooleanOption(options, key, defaultValue = false) {
...
function getNumberOption(options, key, defaultValue) {
    ...
    if (isNaN(value)) {
        logging_1.logger.warn(`Valor inválido para opção ${key}: ${options[key]}. Usando valor padrão: ${defaultValue}`);
        return defaultValue;
    }
    return value;
}
 ...
 */
function exibirAjuda(scriptName, opcoesEspecificas = '', exemplosEspecificos = '') {
    const opcoesComuns = `
