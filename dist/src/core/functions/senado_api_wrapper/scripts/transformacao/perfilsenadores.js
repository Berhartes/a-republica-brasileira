...
exports.perfilSenadoresTransformer = exports.PerfilSenadoresTransformer = void 0;
/**
 ...
 */
class PerfilSenadoresTransformer {
    /**
     ...
     */
    transformSenadoresLegislatura(extractionResult, legislaturaNumero) {
        logging_1.logger.info('Transformando dados de senadores de legislatura específica');
        const { senadores, metadados } = extractionResult;
        if (!senadores || !Array.isArray(senadores) || senadores.length === 0) {
            logging_1.logger.warn('Dados de senadores inválidos ou vazios para transformação');
            return {
        ...
        const senadoresTransformados = senadores.map(senador => {
            try {
                ...
                logging_1.logger.warn(`Erro ao transformar senador básico: ${error.message}`);
        ...
        logging_1.logger.info(`Transformados ${senadoresTransformados.length} senadores da legislatura ${legislatura}`);
        return {
            timestamp: new Date().toISOString(),
    ...
    transformSenadorBasico(senador) {
        ...
        if (!senador || !senador.IdentificacaoParlamentar) {
            logging_1.logger.warn('Dados incompletos de senador para transformação básica');
            return null;
        }
    ...
    transformPerfilCompleto(perfilCompleto) {
        try {
            ...
            if (!perfilCompleto) {
                logging_1.logger.error(`Perfil completo é nulo ou indefinido`);
                return null;
            }
            ...
            if (!perfilCompleto.dadosBasicos ||
                ...
                logging_1.logger.warn(`Dados básicos incompletos ou vazios para o senador ${perfilCompleto.codigo || 'desconhecido'}`);
            ...
            logging_1.logger.info(`Transformando perfil completo do senador ${perfilCompleto.codigo}`);
            ...
            logging_1.logger.debug(`Estrutura completa dos dados básicos: ${JSON.stringify(dadosBasicos)}`);
            ...
            logging_1.logger.debug(`Estrutura de dados parlamentares: ${JSON.stringify(Object.keys(parlamentar))}`);
            // Verificar se temos telefones no nível superior
            let telefonesNivelSuperior = [];
            if (dadosBasicos.Telefones) {
                logging_1.logger.debug(`Telefones encontrados no nível superior: ${JSON.stringify(dadosBasicos.Telefones)}`);
            ...
            if (parlamentar.IdentificacaoParlamentar) {
                logging_1.logger.debug(`Estrutura de identificação: ${JSON.stringify(Object.keys(parlamentar.IdentificacaoParlamentar))}`);
                // Verificar se temos dados de telefone
                if (parlamentar.IdentificacaoParlamentar.Telefones) {
                    logging_1.logger.debug(`Telefones encontrados em parlamentar.IdentificacaoParlamentar.Telefones`);
                }
                // Verificar se temos dados de gênero
                if (parlamentar.IdentificacaoParlamentar.SexoParlamentar) {
                    logging_1.logger.debug(`Gênero encontrado: ${parlamentar.IdentificacaoParlamentar.SexoParlamentar}`);
                }
            }
            if (!parlamentar || Object.keys(parlamentar).length === 0) {
                logging_1.logger.warn(`Dados parlamentares insuficientes para o senador ${perfilCompleto.codigo}`);
            ...
            if (parlamentar.Telefones) {
                logging_1.logger.debug(`Adicionando telefones do parlamentar para transformação: ${JSON.stringify(parlamentar.Telefones)}`);
                ...
                if (telefonesTransformados.length > 0) {
                    ...
                    logging_1.logger.debug(`Adicionados ${telefonesTransformados.length} telefones do parlamentar`);
                }
            }
            // Log para verificar a estrutura completa do parlamentar
            logging_1.logger.debug(`Estrutura completa do parlamentar: ${JSON.stringify(parlamentar)}`);
            ...
            if (dadosBasicos.DetalheParlamentar && dadosBasicos.DetalheParlamentar.Parlamentar && dadosBasicos.DetalheParlamentar.Parlamentar.Telefones) {
                logging_1.logger.debug(`Telefones encontrados em DetalheParlamentar.Parlamentar.Telefones: ${JSON.stringify(dadosBasicos.DetalheParlamentar.Parlamentar.Telefones)}`);
                ...
                if (telefonesDetalheParlamentar.length > 0) {
                    ...
                    logging_1.logger.debug(`Adicionados ${telefonesDetalheParlamentar.length} telefones de DetalheParlamentar.Parlamentar.Telefones`);
                }
            }
            const identificacao = this.transformIdentificacao(parlamentar.IdentificacaoParlamentar ||
                parlamentar.Identificacao || {}, telefonesParaTransformar);
            // 2. Transformar dados pessoais
            const dadosPessoais = this.transformDadosPessoais(parlamentar.DadosBasicosParlamentar || {});
            ...
            logging_1.logger.error(`Erro ao transformar perfil completo do senador ${perfilCompleto?.codigo || 'desconhecido'}: ${error.message}`);
    ...
    transformTelefones(telefones) {
        if (!telefones) {
            logging_1.logger.debug('Dados de telefones não encontrados ou vazios');
            return [];
        }
        // Log detalhado da estrutura de telefones para depuração
        logging_1.logger.debug(`Estrutura de telefones recebida: ${JSON.stringify(telefones)}`);
        // Verificar se temos a estrutura esperada (conforme XML: <Telefones><Telefone>...</Telefone></Telefones>)
        if (!telefones.Telefone) {
            logging_1.logger.debug('Estrutura de telefones não contém o campo Telefone');
            // Tentar encontrar telefones em uma estrutura alternativa
            if (Array.isArray(telefones)) {
                logging_1.logger.debug('Tentando processar array de telefones diretamente');
                return telefones.map((tel) => this.transformTelefoneIndividual(tel));
            }
            ...
            for (const key in telefones) {
                if (typeof telefones[key] === 'object' && telefones[key] !== null) {
                    if (telefones[key].Telefone) {
                        logging_1.logger.debug(`Encontrado campo Telefone em ${key}`);
            ...
            if (telefones.NumeroTelefone) {
                logging_1.logger.debug(`O próprio objeto parece ser um telefone: ${JSON.stringify(telefones)}`);
        ...
        const telefonesArray = Array.isArray(telefones.Telefone)
            ? telefones.Telefone
            : [telefones.Telefone];
        logging_1.logger.debug(`Encontrados ${telefonesArray.length} telefones para transformação`);
        // Log detalhado de cada telefone
        telefonesArray.forEach((tel, index) => {
            logging_1.logger.debug(`Telefone ${index + 1} para transformação: ${JSON.stringify(tel)}`);
        });
        // Transformar cada telefone e filtrar os inválidos
        const telefonesTransformados = telefonesArray
            .map((tel) => this.transformTelefoneIndividual(tel))
            .filter((tel) => tel && tel.numero); // Filtrar telefones sem número
        logging_1.logger.debug(`Transformados ${telefonesTransformados.length} telefones válidos`);
     ...
     */
    transformTelefoneIndividual(tel) {
        // Adicionar log para depuração
        logging_1.logger.debug(`Transformando telefone individual: ${JSON.stringify(tel)}`);
        // Se o objeto for uma string, assumir que é o número do telefone
        if (typeof tel === 'string') {
            logging_1.logger.debug(`Telefone recebido como string: ${tel}`);
            return {
        ...
        if (!tel || Object.keys(tel).length === 0) {
            logging_1.logger.debug('Objeto de telefone vazio ou nulo');
            return {
                numero: '',
                tipo: 'Telefone',
                ordem: undefined
     ...
     */
    transformIdentificacao(identificacao, telefonesAdicionais = []) {
        if (!identificacao || Object.keys(identificacao).length === 0) {
            logging_1.logger.warn('Dados de identificação do parlamentar não encontrados ou vazios');
            return {
                codigo: 'desconhecido',
                nome: 'Dados indisponíveis',
                nomeCompleto: 'Dados indisponíveis',
        ...
        logging_1.logger.debug(`Gênero do parlamentar: ${genero}`);
        // Extrair telefones
        let telefones = [...telefonesAdicionais]; // Iniciar com telefones do nível superior
        try {
            ...
            logging_1.logger.debug(`Estrutura completa de identificação: ${JSON.stringify(identificacao)}`);
            // Verificar diretamente se temos telefones no caminho principal
            if (identificacao.Telefones && identificacao.Telefones.Telefone) {
                logging_1.logger.debug(`Telefones encontrados diretamente em identificacao.Telefones: ${JSON.stringify(identificacao.Telefones)}`);
            }
            ...
            for (const caminho of possiveisCaminhos) {
                ...
                if (telefonesObj) {
                    logging_1.logger.debug(`Dados de telefones encontrados em ${caminho.desc}`);
                    ...
                    if (telefonesTransformados.length > 0) {
                        ...
                        logging_1.logger.debug(`Adicionados ${telefonesTransformados.length} telefones de ${caminho.desc}`);
                    }
                }
            }
            // Se não encontrou telefones em nenhum caminho conhecido, registrar
            if (!telefonesEncontrados && telefones.length === 0) {
                logging_1.logger.debug(`Nenhum dado de telefone encontrado nas estruturas conhecidas além dos telefones adicionais`);
                ...
                for (const key in identificacao) {
                    if (typeof identificacao[key] === 'object' && identificacao[key] !== null) {
                        ...
                        if (key.toLowerCase().includes('telefone')) {
                            logging_1.logger.debug(`Possível campo de telefone encontrado em identificacao.${key}`);
                            ...
                            if (possiveisTelefones.length > 0) {
                                ...
                                logging_1.logger.debug(`Adicionados ${possiveisTelefones.length} telefones de identificacao.${key}`);
                            }
                        }
                    }
                }
            ...
            logging_1.logger.warn(`Erro ao processar telefones: ${error.message}`);
        }
        return {
     ...
     */
    transformDadosPessoais(dadosBasicos) {
        if (!dadosBasicos || Object.keys(dadosBasicos).length === 0) {
            logging_1.logger.warn('Dados pessoais do parlamentar não encontrados ou vazios');
            return {
                dataNascimento: null,
                naturalidade: '',
                ufNaturalidade: ''
     ...
     */
    transformMandatosCompletos(mandatos) {
        // Se não temos dados válidos, retornar array vazio
        if (!mandatos || !mandatos.Parlamentar || !mandatos.Parlamentar.Mandatos) {
            logging_1.logger.warn('Dados de mandatos do parlamentar não encontrados ou vazios');
            return [];
        }
        ...
        return mandatosArray.map((mandato) => {
            if (!mandato)
                return null;
    ...
    transformExercicios(exercicios) {
        ...
        const exerciciosArray = Array.isArray(exercicios.Exercicio)
            ...
            : [exercicios.Exercicio];
        return exerciciosArray.map((exercicio) => ({
    ...
    transformSuplentes(suplentes) {
        ...
        const suplentesArray = Array.isArray(suplentes.Suplente)
            ? suplentes.Suplente
            : [suplentes.Suplente];
        return suplentesArray.map((suplente) => ({
     ...
     */
    transformCargos(cargos) {
        if (!cargos || !cargos.Parlamentar || !cargos.Parlamentar.Cargos) {
            logging_1.logger.warn('Dados de cargos do parlamentar não encontrados ou vazios');
            return [];
        }
     ...
     */
    transformComissoes(comissoes) {
        if (!comissoes || !comissoes.Parlamentar || !comissoes.Parlamentar.MembroComissoes) {
            logging_1.logger.warn('Dados de comissões do parlamentar não encontrados ou vazios');
            return [];
        }
        ...
        return comissoesArray.map((comissao) => {
            if (!comissao || !comissao.IdentificacaoComissao) {
                logging_1.logger.warn('Comissão sem identificação encontrada, pulando...');
                return null;
            }
     ...
     */
    transformFiliacoes(filiacoes) {
        if (!filiacoes || !filiacoes.Parlamentar || !filiacoes.Parlamentar.Filiacoes) {
            logging_1.logger.warn('Dados de filiações do parlamentar não encontrados ou vazios');
            return [];
        }
        ...
        return filiacoesArray.map((filiacao) => {
            if (!filiacao || !filiacao.Partido) {
                logging_1.logger.warn('Filiação sem partido encontrada, pulando...');
                return null;
            }
     ...
     */
    transformHistoricoAcademico(historicoAcademico) {
        // Verificar se temos dados válidos
        if (!historicoAcademico) {
            logging_1.logger.warn('Dados de histórico acadêmico do parlamentar não encontrados ou vazios');
            return [];
        }
        // Verificar se temos dados do parlamentar
        if (!historicoAcademico.Parlamentar) {
            logging_1.logger.warn('Dados do parlamentar não encontrados no histórico acadêmico');
            return [];
        }
        // Verificar se temos dados de histórico acadêmico
        if (!historicoAcademico.Parlamentar.HistoricoAcademico) {
            logging_1.logger.warn('Histórico acadêmico do parlamentar não encontrado');
            return [];
        }
        ...
        if (!historicoObj.Curso) {
            logging_1.logger.warn('Nenhum curso encontrado no histórico acadêmico do parlamentar');
            return [];
        }
        // Converter para array se necessário
     ...
     */
    transformLicencas(licencas) {
        if (!licencas || !licencas.Parlamentar || !licencas.Parlamentar.Licencas) {
            logging_1.logger.warn('Dados de licenças do parlamentar não encontrados ou vazios');
            return [];
        }
        ...
        return licencasArray.map((licenca) => {
            if (!licenca)
                return null;
    ...
    transformProfissao(profissao) {
        ...
        if (!profissao ||
            ...
            logging_1.logger.warn('Dados de profissão do parlamentar não encontrados ou vazios');
            return [];
        }
        ...
        return profissoesArray.map((prof) => {
            if (!prof)
                return null;
            return {
    ...
    transformApartes(apartes) {
        if (!apartes || !apartes.Parlamentar || !apartes.Parlamentar.Apartes) {
            logging_1.logger.warn('Dados de apartes do parlamentar não encontrados ou vazios');
            return [];
        }
     ...
     */
    transformLiderancas(liderancas) {
        if (!liderancas) {
            logging_1.logger.warn('Dados de lideranças não encontrados ou vazios');
            return [];
        }
        try {
            // Adicionar log para verificar a estrutura completa da resposta
            logging_1.logger.debug(`Estrutura completa da resposta de lideranças: ${JSON.stringify(Object.keys(liderancas))}`);
            ...
            if (liderancas.ListaLiderancas && liderancas.ListaLiderancas.Liderancas && liderancas.ListaLiderancas.Liderancas.Lideranca) {
                logging_1.logger.debug('Estrutura encontrada: liderancas.ListaLiderancas.Liderancas.Lideranca');
                ...
                logging_1.logger.debug('Estrutura encontrada: liderancas.liderancas.lideranca');
                listaLiderancas = liderancas.liderancas.lideranca;
            }
            else if (liderancas.liderancas) {
                logging_1.logger.debug('Estrutura encontrada: liderancas.liderancas');
                listaLiderancas = liderancas.liderancas;
            }
            else {
                // Procurar em todas as chaves possíveis
                const procurarLiderancas = (obj) => {
                ...
                logging_1.logger.debug(`Encontradas ${listaLiderancas.length} lideranças através de busca recursiva`);
            }
            ...
            if (listaLiderancas.length > 0) {
                logging_1.logger.debug(`Estrutura de uma liderança: ${JSON.stringify(Object.keys(listaLiderancas[0]))}`);
            }
            ...
            logging_1.logger.warn(`Erro ao transformar lideranças: ${error.message}`);
            return [];
        }
    }
    /**
     ...
     */
    determinarSituacaoAtual(mandatos, licencas) {
        // Valores padrão
        const situacao = {
            emExercicio: false,
            titular: false,
     ...
     */
    transformPerfilCompletoOrganizado(perfilCompleto) {
        try {
            ...
            const perfilOrganizado = {
                ...
                mandatos: perfilCompleto.mandatos.map(mandato => {
                    ...
                    if ('uf' in mandato) {
                        mandatoOrganizado.uf = mandato.uf;
                    }
                    // Adicionar informações de legislaturas se existirem
                    if ('primeiraLegislatura' in mandato && mandato.primeiraLegislatura) {
                    ...
                    if ('segundaLegislatura' in mandato && mandato.segundaLegislatura) {
            ...
            logging_1.logger.error(`Erro ao transformar perfil completo organizado: ${error.message}`);
            return null;
        }
    }
}
exports.PerfilSenadoresTransformer = PerfilSenadoresTransformer;
exports.perfilSenadoresTransformer = new PerfilSenadoresTransformer();
