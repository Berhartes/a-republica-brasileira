...
exports.materiasTransformer = exports.MateriasTransformer = void 0;
/**
...
class MateriasTransformer {
     ...
     */
    transformMateria(materiaResult) {
        try {
            logger_1.logger.info(`Transformando matérias do senador ${materiaResult.codigo}`);
            // Verificar se temos dados válidos
            if (!materiaResult || materiaResult.erro || !materiaResult.dadosBasicos || !materiaResult.dadosBasicos.dados) {
                logger_1.logger.warn(`Dados inválidos para o senador ${materiaResult.codigo}, pulando transformação`);
                return null;
            }
            ...
            logger_1.logger.info(`Estrutura completa dos dados básicos do senador ${materiaResult.codigo}: ${JSON.stringify(Object.keys(dadosBasicos), null, 2)}`);
            ...
            if (dadosBasicos.DetalheParlamentar?.Parlamentar) {
                ...
                logger_1.logger.info(`Dados do parlamentar encontrados no caminho DetalheParlamentar.Parlamentar`);
            }
            // Caminho 2: Parlamentar
            else if (dadosBasicos.Parlamentar) {
                parlamentar = dadosBasicos.Parlamentar;
                identificacao = parlamentar.IdentificacaoParlamentar || {};
                logger_1.logger.info(`Dados do parlamentar encontrados no caminho Parlamentar`);
            }
            // Caminho 3: IdentificacaoParlamentar diretamente
                ...
                logger_1.logger.info(`Dados do parlamentar encontrados no caminho IdentificacaoParlamentar`);
            }
            // Log para depuração dos dados de identificação
            logger_1.logger.info(`Dados de identificação do senador ${materiaResult.codigo}: ${JSON.stringify(identificacao, null, 2)}`);
            // Verificar se temos o nome do senador
            if (!identificacao.NomeParlamentar) {
                logger_1.logger.warn(`Nome do senador ${materiaResult.codigo} não encontrado nos dados básicos. Tentando extrair de outras fontes.`);
            }
            ...
            if (!nomeSenador) {
                ...
                if (dadosBasicos.DetalheParlamentar?.Parlamentar?.NomeParlamentar) {
                    ...
                    logger_1.logger.info(`Nome do senador extraído de DetalheParlamentar.Parlamentar.NomeParlamentar: ${nomeSenador}`);
                }
                    ...
                    logger_1.logger.info(`Nome do senador extraído de DetalheParlamentar.Parlamentar.IdentificacaoParlamentar.NomeParlamentar: ${nomeSenador}`);
                }
                else if (dadosBasicos.Parlamentar?.NomeParlamentar) {
                    nomeSenador = dadosBasicos.Parlamentar.NomeParlamentar;
                    logger_1.logger.info(`Nome do senador extraído de Parlamentar.NomeParlamentar: ${nomeSenador}`);
                }
                    ...
                    logger_1.logger.info(`Nome do senador extraído de Parlamentar.IdentificacaoParlamentar.NomeParlamentar: ${nomeSenador}`);
                }
                    ...
                    logger_1.logger.info(`Nome do senador extraído de IdentificacaoParlamentar.NomeParlamentar: ${nomeSenador}`);
                }
                    ...
                    logger_1.logger.info(`Nome do senador extraído de DadosBasicosParalamentar.NomeParlamentar: ${nomeSenador}`);
                }
            }
            else {
                logger_1.logger.info(`Nome do senador extraído de identificacao.NomeParlamentar: ${nomeSenador}`);
            }
            // Se ainda não encontrou, usar um valor padrão
            if (!nomeSenador) {
                nomeSenador = 'Nome não disponível';
                logger_1.logger.warn(`Não foi possível extrair o nome do senador ${materiaResult.codigo} de nenhuma fonte`);
            }
            ...
            // Log para depuração
            logger_1.logger.info(`Autorias separadas por tipo:`);
            logger_1.logger.info(`- Individuais: ${autoriasIndividuais.length}`);
            logger_1.logger.info(`- Coautorias: ${coautorias.length}`);
            logger_1.logger.info(`- Coletivas: ${autoriasColetivas.length}`);
            ...
            if (estatisticasAutorias.porTipo) {
                logger_1.logger.info(`Estatísticas por tipo de matéria:`);
                Object.entries(estatisticasAutorias.porTipo).forEach(([tipo, stats]) => {
                    logger_1.logger.info(`- ${tipo}: ${stats.total} (${stats.percentual}%)`);
                });
            }
            ...
            if (estatisticasAutorias.parceirosFrequentes && estatisticasAutorias.parceirosFrequentes.length > 0) {
                logger_1.logger.info(`Parceiros frequentes em coautorias:`);
                estatisticasAutorias.parceirosFrequentes.forEach((parceiro, index) => {
                    logger_1.logger.info(`${index + 1}. ${parceiro.nome}: ${parceiro.ocorrencias} ocorrências (${parceiro.percentual}%)`);
                });
            }
            ...
            logger_1.logger.error(`Erro ao transformar matérias do senador ${materiaResult.codigo}: ${error.message}`);
            return null;
        }
    }
    /**
     ...
     */
    async transformMultiplasMaterias(materiasResults) {
        try {
            logger_1.logger.info(`Transformando ${materiasResults.length} matérias de senadores`);
            ...
            for (const materiaResult of materiasResults) {
                try {
                    ...
                    logger_1.logger.warn(`Erro ao transformar matéria do senador ${materiaResult.codigo}: ${error.message}`);
                }
            }
            logger_1.logger.info(`Transformação concluída: ${materiasTransformadas.length} matérias transformadas`);
            return materiasTransformadas;
        }
        catch (error) {
            logger_1.logger.error(`Erro ao transformar múltiplas matérias: ${error.message}`);
            return [];
        }
    }
    /**
     ...
     */
    extrairParceirosCoautorias(coautorias, nomeSenador) {
        ...
        for (const coautoria of coautorias) {
            if (!coautoria || !coautoria.autoria)
                continue;
    ...
    categorizarPorTipo(autorias) {
        ...
        for (const autoria of autorias) {
            if (!autoria || !autoria.tipo)
                continue;
            // Normalizar o tipo (ex: "PEC", "PL", "REQ")
            let tipo = autoria.tipo.toUpperCase().trim();
            // Agrupar alguns tipos similares
            if (tipo.includes('REQ'))
                tipo = 'REQ';
            if (tipo.includes('PEC'))
                tipo = 'PEC';
            if (tipo.includes('PL'))
                tipo = 'PL';
            if (tipo.includes('PLS'))
                tipo = 'PLS';
            if (tipo.includes('MPV'))
                tipo = 'MPV';
     ...
     */
    calcularEstatisticasAutorias(autorias, coautorias, nomeSenador) {
        if (!autorias || autorias.length === 0) {
            return {
                total: 0,
                individual: 0,
     ...
     */
    transformarAutorias(autoriasResult) {
        try {
            if (!autoriasResult || !autoriasResult.dados) {
                logger_1.logger.warn('Dados de autorias não encontrados ou inválidos');
                return [];
            }
            // Log para depuração
            logger_1.logger.info(`Estrutura de dados de autorias: ${JSON.stringify(Object.keys(autoriasResult.dados), null, 2)}`);
            ...
            if (!processos) {
                logger_1.logger.warn('Nenhum processo encontrado nos dados de autorias. Estrutura completa:', autoriasResult.dados);
                return [];
            }
            // Garantir que processos seja um array
            const processosArray = Array.isArray(processos) ? processos : [processos];
            logger_1.logger.info(`Encontrados ${processosArray.length} processos de autoria`);
            // Obter o código do senador dos metadados
            const codigoSenador = autoriasResult.metadados?.senadorCodigo || '';
            if (codigoSenador) {
                logger_1.logger.info(`Processando autorias para o senador código ${codigoSenador}`);
            }
            else {
                logger_1.logger.warn('Código do senador não encontrado nos metadados');
            }
            // Transformar cada autoria
            const autoriasTransformadas = processosArray.map(processo => {
                try {
                    // Log para depuração
                    logger_1.logger.debug(`Processando processo: ${JSON.stringify(processo, null, 2)}`);
                    ...
                    logger_1.logger.warn(`Erro ao processar autoria: ${error.message}`);
                    return null;
                }
            ...
            if (autoriasTransformadas.length > 0) {
                ...
                logger_1.logger.info(`Estatísticas de autorias para o senador ${codigoSenador || 'desconhecido'}:`);
                logger_1.logger.info(`- Total: ${estatisticas.total}`);
                logger_1.logger.info(`- Individuais: ${estatisticas.individual} (${Math.round(estatisticas.individual / estatisticas.total * 100)}%)`);
                logger_1.logger.info(`- Coautorias: ${estatisticas.coautoria} (${Math.round(estatisticas.coautoria / estatisticas.total * 100)}%)`);
                logger_1.logger.info(`- Coletivas: ${estatisticas.coletiva} (${Math.round(estatisticas.coletiva / estatisticas.total * 100)}%)`);
                if (estatisticas.outro > 0) {
                    logger_1.logger.info(`- Outros tipos: ${estatisticas.outro} (${Math.round(estatisticas.outro / estatisticas.total * 100)}%)`);
                }
                if (estatisticas.desconhecido > 0) {
                    logger_1.logger.info(`- Desconhecidos: ${estatisticas.desconhecido} (${Math.round(estatisticas.desconhecido / estatisticas.total * 100)}%)`);
                }
            }
            return autoriasTransformadas;
        }
        catch (error) {
            logger_1.logger.error(`Erro ao transformar autorias: ${error.message}`);
            return [];
        }
    }
    /**
     ...
     */
    transformarRelatorias(relatoriasResult) {
        try {
            if (!relatoriasResult || !relatoriasResult.dados) {
                logger_1.logger.warn('Dados de relatorias não encontrados ou inválidos');
                return [];
            }
            // Log para depuração
            logger_1.logger.info(`Estrutura de dados de relatorias: ${JSON.stringify(Object.keys(relatoriasResult.dados), null, 2)}`);
            ...
            if (!relatorias) {
                logger_1.logger.warn('Nenhuma relatoria encontrada nos dados');
                return [];
            }
            // Garantir que relatorias seja um array
            const relatoriasArray = Array.isArray(relatorias) ? relatorias : [relatorias];
            logger_1.logger.info(`Encontradas ${relatoriasArray.length} relatorias`);
            // Transformar cada relatoria
            return relatoriasArray.map(relatoria => {
                try {
                    // Log para depuração
                    logger_1.logger.debug(`Processando relatoria: ${JSON.stringify(relatoria, null, 2)}`);
                    ...
                    logger_1.logger.warn(`Erro ao processar relatoria: ${error.message}`);
                    return null;
                }
            }).filter(relatoria => relatoria && relatoria.id); // Filtrar relatorias sem ID ou nulas
        }
        catch (error) {
            logger_1.logger.error(`Erro ao transformar relatorias: ${error.message}`);
            return [];
        }
    }
}
exports.MateriasTransformer = MateriasTransformer;
// Instância singleton para uso em toda a aplicação
exports.materiasTransformer = new MateriasTransformer();
