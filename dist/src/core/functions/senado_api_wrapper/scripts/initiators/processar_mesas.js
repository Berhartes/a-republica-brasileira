...
const mesas_processor_1 = require("../processors/mesas.processor");
...
async function main() {
    ...
    try {
        ...
        const cli = new etl_cli_1.ETLCommandParser('senado:mesas', 'Processa mesas diretoras do Senado Federal');
        // Adicionar opções customizadas específicas deste script
        cli.addCustomOption('--incluir-composicoes', () => true);
        cli.addCustomOption('--periodo', (value) => value);
        ...
        exibirBanner();
        // Criar processador
        const processor = new mesas_processor_1.MesasProcessor(options);
        ...
        logging_1.logger.info('Iniciando processamento de mesas diretoras...');
...
`);
    ...
    if (resultado.detalhes && Array.isArray(resultado.detalhes)) {
        ...
        const mesas = resultado.detalhes.find((d) => d.id === 'mesas');
        const membros = resultado.detalhes.find((d) => d.id === 'composicao' || d.id === 'membros');
        if (mesas) {
...
//# sourceMappingURL=processar_mesas.js.map