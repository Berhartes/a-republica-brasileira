...
exports.DespesasDeputadosProcessor = void 0;
...
const etl_config_1 = require("../config/etl.config"); // Remover processadorConfigs
 ...
 */
class DespesasDeputadosProcessor extends etl_processor_1.ETLProcessor {
    constructor(options) {
        super(options);
    }
    /**
    ...
    async extract() {
        ...
        try {
            ...
            this.emitProgress(etl_types_1.ProcessingStatus.EXTRAINDO, 30, `Extraindo despesas de ${deputadosParaProcessar.length} deputados`);
     ...
     */
    async extractDespesasDeputados(deputados, modoAtualizacao = false) {
        const resultados = [];
        ...
        for (let i = 0; i < deputados.length; i += concorrencia) {
            ...
            this.emitProgress(etl_types_1.ProcessingStatus.EXTRAINDO, progresso, `${resultados.length}/${deputados.length} deputados processados (${totalDespesas} despesas)`);
            // Pausa entre lotes
     ...
     */
    async extractDespesasCompletas(deputadoId) {
        const legislatura = this.context.options.legislatura;
        ...
        try {
            ...
            const baseParams = {
                ...
                idLegislatura: legislatura.toString(),
                itens: String(etl_config_1.etlConfig.camara.itemsPerPage || endpoints_1.endpoints.REQUEST.DEFAULT_ITEMS_PER_PAGE) // Use etlConfig.camara.itemsPerPage
            };
            // Adicionar filtros de período se especificados
            if (ano)
                baseParams.ano = ano.toString(); // Ensure it's a string
            if (mes)
                baseParams.mes = mes.toString(); // Ensure it's a string
            // Usar getAllPages para extrair todas as páginas automaticamente
            const todasDespesas = await api_1.apiClient.getAllPages(endpoint, baseParams, {
                context: `Despesas do deputado ${deputadoId}`,
                maxPages: 100 // Limite de segurança
            });
            const valorTotal = todasDespesas.reduce((sum, despesa) => {
            ...
            return {
                ...
                totalPaginas: Math.ceil(todasDespesas.length / (etl_config_1.etlConfig.camara.itemsPerPage || endpoints_1.endpoints.REQUEST.DEFAULT_ITEMS_PER_PAGE)), // Use etlConfig.camara.itemsPerPage
                valorTotal
            };
        }
        catch (error) {
     ...
     */
    async extractDespesasIncremental(deputadoId) {
        ...
        const mesesUnicos = mesesParaVerificar.filter((mes, index, array) => array.findIndex(m => m.ano === mes.ano && m.mes === mes.mes) === index);
        ...
        for (const { ano, mes } of mesesUnicos) {
            try {
                ...
                const despesasRecentes = despesasMes.despesas.filter((despesa) => {
     ...
     */
    async extractDespesasPorMes(deputadoId, ano, mes) {
        const legislatura = this.context.options.legislatura;
        const endpointConfig = endpoints_1.endpoints.DEPUTADOS.DESPESAS;
        const endpoint = (0, api_1.replacePath)(endpointConfig.PATH, { codigo: deputadoId });
        const params = {
        ...
        const despesas = await api_1.apiClient.getAllPages(endpoint, params, {
            context: `Despesas ${ano}-${mes.toString().padStart(2, '0')} do deputado ${deputadoId}`,
            maxPages: 20
        });
        return {
            despesas,
            totalPaginas: Math.ceil(despesas.length / (etl_config_1.etlConfig.camara.itemsPerPage || endpoints_1.endpoints.REQUEST.DEFAULT_ITEMS_PER_PAGE)) // Use etlConfig.camara.itemsPerPage
        };
    }
    /**
    ...
    async transform(data) {
        ...
        try {
            ...
            for (const dadosDeputado of data.despesasPorDeputado) {
                ...
                this.emitProgress(etl_types_1.ProcessingStatus.TRANSFORMANDO, progresso, `${despesasTransformadas.length} despesas transformadas`);
            }
            const estatisticas = {
     ...
     */
    transformDespesa(despesaBruta, deputadoId) {
        return {
    ...
    async load(data) {
        ...
        let despesasPorDeputado = {}; // Declarar fora do try
        let batchResults; // Pode não ser definido se houver erro antes do commit
        const modoAtualizacao = this.context.options.atualizar || false;
        const batchManager = new config_1.FirestoreBatchManager(); // Usar diretamente
        try {
            this.emitProgress(etl_types_1.ProcessingStatus.CARREGANDO, 30, 'Salvando despesas por deputado');
            ...
            for (const [deputadoId, novasDespesasDoDeputado] of Object.entries(despesasPorDeputado)) {
                ...
                const novasDespesasAgrupadasPorAno = {};
                ...
                let todasAsDespesasDoDeputadoParaEstatisticas = [];
                ...
                for (const [ano, novasDespesasDoAno] of Object.entries(novasDespesasAgrupadasPorAno)) {
                    ...
                    if (modoAtualizacao) {
                        ...
                        let despesasExistentes = [];
                        try {
                            const docRef = config_1.db.doc(pathDespesasAno);
                            const docSnap = await docRef.get();
                            if (docSnap.exists) {
                                const docData = docSnap.data();
                                if (docData && docData.despesas && Array.isArray(docData.despesas)) {
                                    despesasExistentes = docData.despesas;
                                }
                            }
                        }
                        catch (e) {
                        ...
                        const mapDespesasUnicas = new Map();
                    ...
                    const { collectionPath: colPath, documentId: docId } = (0, helpers_1.parseFirestorePath)(basePathDeputado, ano);
                    await batchManager.set(colPath, docId, { despesas: despesasFinaisParaSalvar });
                ...
                if (todasAsDespesasDoDeputadoParaEstatisticas.length > 0) {
                    ...
                    const { collectionPath: colPathMain, documentId: docIdMain } = (0, helpers_1.parseFirestorePath)(`congressoNacional/CamaraDeputados/despesas`, deputadoId);
                    await batchManager.set(colPathMain, docIdMain, dadosEstatisticasDeputado);
                }
            ...
            const metadataCollectionPath = `congressoNacional/CamaraDeputados/despesas`;
            ...
            // O commit do FirestoreBatchManager não retorna BatchResult diretamente.
            // A lógica de sucesso/falha é gerenciada internamente pelo BatchManager.
            // Precisamos construir o ETLResult com base nas estatísticas do processador.
            await batchManager.commit();
            // Contar sucessos e falhas com base no que foi tentado e o que o batch manager pode ter logado.
            // Esta parte pode precisar de ajuste fino se o batchManager não relançar erros de commit.
            // Por enquanto, vamos assumir que se chegou aqui sem erro, a maioria foi sucesso.
            // O BatchManager já loga erros de commit.
            const totalOperacoesEstimadas = data.despesas.length + Object.keys(despesasPorDeputado).length + 1; // despesas + deputados + metadata
            const finalResult = {
                sucessos: this.context.stats.carregamento.sucesso || totalOperacoesEstimadas, // Assumir sucesso se não houver erro
                falhas: this.context.stats.carregamento.falha || 0,
                avisos: this.context.stats.avisos,
                tempoProcessamento: Date.now() - this.context.stats.inicio, // Tempo total do processador
                tempoCarregamento: Date.now() - inicioCarregamento, // Usar o timestamp de início do carregamento
     ...
     */
    groupDespesasByDeputado(despesas) {
     ...
     */
    calculateDeputadoStats(despesas) {
        return {
...
exports.DespesasDeputadosProcessor = DespesasDeputadosProcessor;
//# sourceMappingURL=despesas-deputados.processor.js.map