...
exports.PerfilDeputadosProcessor = void 0;
const tslib_1 = require("tslib");
const etl_processor_1 = require("../core/etl-processor");
const etl_types_1 = require("../types/etl.types");
const config_1 = require("../utils/storage/firestore/config"); // Importar FirestoreBatchManager
 ...
 */
class PerfilDeputadosProcessor extends etl_processor_1.ETLProcessor {
    constructor(options) {
        super(options);
        this.rawPerfisCompletos = []; // Adicionar para armazenar perfis crus
    }
    /**
     * Nome do processador
     */
    ...
    async extract() {
        ...
        try {
            ...
            this.emitProgress(etl_types_1.ProcessingStatus.EXTRAINDO, 30, `Extraindo perfis de ${deputadosParaProcessar.length} deputados`);
    ...
    async extractDeputadoEspecifico(deputadoId, legislatura) {
        try {
            ...
            const response = await (0, error_handler_1.withRetry)(() => api.get(endpoint, endpointConfig.PARAMS), etl_config_1.etlConfig.camara.maxRetries, etl_config_1.etlConfig.camara.pauseBetweenRequests, `Perfil do deputado ${deputadoId}`);
            ...
            const pertenceLegislatura = deputado.ultimoStatus?.idLegislatura === legislatura ||
                (deputado.mandatos && deputado.mandatos.some((m) => m.idLegislatura === legislatura));
    ...
    async extractDeputadosLegislatura(legislatura) {
        try {
            ...
            do {
                ...
                const response = await (0, error_handler_1.withRetry)(() => api.get(endpointConfig.PATH, params), etl_config_1.etlConfig.camara.maxRetries, etl_config_1.etlConfig.camara.pauseBetweenRequests, `Lista de deputados da legislatura ${legislatura}, página ${pagina}`);
     ...
     */
    async extractPerfisCompletos(deputados) {
        const perfis = [];
        const concorrencia = this.context.options.concorrencia || etl_config_1.etlConfig.camara.concurrency;
        ...
        for (let i = 0; i < deputados.length; i += concorrencia) {
            ...
            this.emitProgress(etl_types_1.ProcessingStatus.EXTRAINDO, progresso, `${perfis.length}/${deputados.length} perfis extraídos`);
            // Pausa entre lotes
            if (i + concorrencia < deputados.length) {
                await new Promise(resolve => setTimeout(resolve, etl_config_1.etlConfig.camara.pauseBetweenRequests));
            }
        }
     ...
     */
    async extractPerfilCompleto(deputadoId) {
        const endpointConfig = endpoints_1.endpoints.DEPUTADOS.PERFIL;
        const endpoint = api.replacePath(endpointConfig.PATH, { codigo: deputadoId });
        const response = await (0, error_handler_1.withRetry)(() => api.get(endpoint, endpointConfig.PARAMS), etl_config_1.etlConfig.camara.maxRetries, etl_config_1.etlConfig.camara.pauseBetweenRequests, `Perfil completo do deputado ${deputadoId}`);
        if (!response || !response.dados) {
        ...
        const perfilCompleto = { ...perfilBase };
     ...
     */
    async extractOrgaos(deputadoId) {
        const endpointConfig = endpoints_1.endpoints.DEPUTADOS.ORGAOS;
        const endpoint = api.replacePath(endpointConfig.PATH, { codigo: deputadoId });
        const response = await (0, error_handler_1.withRetry)(() => api.get(endpoint, endpointConfig.PARAMS), etl_config_1.etlConfig.camara.maxRetries, etl_config_1.etlConfig.camara.pauseBetweenRequests, `Órgãos do deputado ${deputadoId}`);
        return response?.dados || [];
    }
    /**
     * Extrai frentes parlamentares de um deputado
     */
    async extractFrentes(deputadoId) {
        const endpointConfig = endpoints_1.endpoints.DEPUTADOS.FRENTES;
        const endpoint = api.replacePath(endpointConfig.PATH, { codigo: deputadoId });
        const response = await (0, error_handler_1.withRetry)(() => api.get(endpoint, endpointConfig.PARAMS), etl_config_1.etlConfig.camara.maxRetries, etl_config_1.etlConfig.camara.pauseBetweenRequests, `Frentes do deputado ${deputadoId}`);
        return response?.dados || [];
    }
    /**
     * Extrai ocupações de um deputado
     */
    async extractOcupacoes(deputadoId) {
        const endpointConfig = endpoints_1.endpoints.DEPUTADOS.OCUPACOES;
        const endpoint = api.replacePath(endpointConfig.PATH, { codigo: deputadoId });
        const response = await (0, error_handler_1.withRetry)(() => api.get(endpoint, endpointConfig.PARAMS), etl_config_1.etlConfig.camara.maxRetries, etl_config_1.etlConfig.camara.pauseBetweenRequests, `Ocupações do deputado ${deputadoId}`);
        return response?.dados || [];
    }
    /**
     * Extrai mandatos externos de um deputado
     */
    async extractMandatosExternos(deputadoId) {
        const endpointConfig = endpoints_1.endpoints.DEPUTADOS.MANDATOS_EXTERNOS;
        const endpoint = api.replacePath(endpointConfig.PATH, { codigo: deputadoId });
        const response = await (0, error_handler_1.withRetry)(() => api.get(endpoint, endpointConfig.PARAMS), etl_config_1.etlConfig.camara.maxRetries, etl_config_1.etlConfig.camara.pauseBetweenRequests, `Mandatos externos do deputado ${deputadoId}`);
        return response?.dados || [];
    }
    /**
     * Extrai histórico de um deputado
     */
    async extractHistorico(deputadoId) {
        const endpointConfig = endpoints_1.endpoints.DEPUTADOS.HISTORICO;
        const endpoint = api.replacePath(endpointConfig.PATH, { codigo: deputadoId });
        const response = await (0, error_handler_1.withRetry)(() => api.get(endpoint, endpointConfig.PARAMS), etl_config_1.etlConfig.camara.maxRetries, etl_config_1.etlConfig.camara.pauseBetweenRequests, `Histórico do deputado ${deputadoId}`);
        return response?.dados || [];
    }
    /**
     * Extrai profissões de um deputado
     */
    async extractProfissoes(deputadoId) {
        const endpointConfig = endpoints_1.endpoints.DEPUTADOS.PROFISSOES;
        const endpoint = api.replacePath(endpointConfig.PATH, { codigo: deputadoId });
        const response = await (0, error_handler_1.withRetry)(() => api.get(endpoint, endpointConfig.PARAMS), etl_config_1.etlConfig.camara.maxRetries, etl_config_1.etlConfig.camara.pauseBetweenRequests, `Profissões do deputado ${deputadoId}`);
        return response?.dados || [];
    }
    /**
    ...
    async transform(data) {
        ...
        try {
            ...
            for (let i = 0; i < data.perfisCompletos.length; i++) {
                ...
                this.emitProgress(etl_types_1.ProcessingStatus.TRANSFORMANDO, progresso, `${i + 1}/${data.perfisCompletos.length} perfis transformados`);
            }
            const estatisticas = {
    ...
    transformPerfil(perfilBruto) {
        ...
        if (perfilBruto.mandatos && Array.isArray(perfilBruto.mandatos)) {
            perfil.mandatos = perfilBruto.mandatos.map((mandato) => ({
        ...
        if (perfilBruto.filiacoes && Array.isArray(perfilBruto.filiacoes)) {
            perfil.filiacoes = perfilBruto.filiacoes.map((filiacao) => ({
    ...
    async load(data) {
        ...
        const startTime = Date.now(); // Manter startTime para cálculo de duração se necessário, ou usar inicioCarregamento
        ...
        let documentosSalvos = 0; // Este contador pode ser substituído pela lógica interna do batchManager
        try {
            // 1. Salvar dados básicos da API (raw response) no caminho de "perfil"
            this.emitProgress(etl_types_1.ProcessingStatus.CARREGANDO, 20, 'Salvando dados brutos do perfil (API detalhes)');
            if (this.rawPerfisCompletos && this.rawPerfisCompletos.length > 0) {
                for (const rawPerfil of this.rawPerfisCompletos) {
                    if (rawPerfil && rawPerfil.id) {
                        const { collectionPath, documentId } = (0, helpers_1.parseFirestorePath)('congressoNacional/CamaraDeputados/perfil', rawPerfil.id.toString());
                        await batchManager.set(collectionPath, documentId, rawPerfil);
                        // documentosSalvos++; // O batchManager controla a contagem
                    }
                }
            }
            // 2. Salvar perfis completos transformados no caminho de "legislatura"
            this.emitProgress(etl_types_1.ProcessingStatus.CARREGANDO, 50, 'Salvando perfis transformados da legislatura');
            for (const perfil of data.perfis) { // data.perfis são os transformados
                ...
                const { collectionPath, documentId } = (0, helpers_1.parseFirestorePath)(`congressoNacional/CamaraDeputados/legislatura/${idLegislaturaParaSalvar}/deputados`, perfil.id.toString());
                await batchManager.set(collectionPath, documentId, perfil);
                // documentosSalvos++; // O batchManager controla a contagem
            }
            // 3. Salvar metadados da legislatura
            this.emitProgress(etl_types_1.ProcessingStatus.CARREGANDO, 80, 'Salvando metadados da legislatura');
            ...
            const metadataCollection = `congressoNacional/CamaraDeputados/legislatura`;
            const metadataId = `metadata_legislatura_${legislaturaAtual}`;
            const { collectionPath: metaCol, documentId: metaId } = (0, helpers_1.parseFirestorePath)(metadataCollection, metadataId);
            await batchManager.set(metaCol, metaId, metadata);
            // documentosSalvos++; // O batchManager controla a contagem
            // Executar batch
            this.emitProgress(etl_types_1.ProcessingStatus.CARREGANDO, 95, 'Commit das operações');
            await batchManager.commit(); // O commit do FirestoreBatchManager não retorna BatchResult diretamente
            const duration = Date.now() - inicioCarregamento; // Usar inicioCarregamento
            // As estatísticas de load (sucesso/falha) são atualizadas internamente pelo ETLProcessor
            // com base nos incrementos de sucesso/falha ou no resultado do batch se ele retornasse.
            // Como o batchManager agora lida com isso internamente e loga,
            // podemos confiar nos logs do batchManager para detalhes de falhas de commit.
            // O ETLResult do processador refletirá o sucesso geral.
            // Estimativa de operações para o log, já que o batchManager não retorna contagem.
            const operacoesEstimadas = (this.rawPerfisCompletos?.length || 0) + data.perfis.length + 1;
            this.context.logger.info(`✅ Carregamento concluído: Aproximadamente ${operacoesEstimadas} operações no batch. Duração: ${duration}ms`);
            // O retorno do método load deve ser compatível com o que o ETLProcessor espera
            // ou o tipo de retorno do process() deve ser ajustado.
            // Por ora, retornando um objeto simples. O ETLResult é construído no método process().
            return {
                sucessos: this.context.stats.carregamento.sucesso || operacoesEstimadas, // Usar estatísticas do contexto
                falhas: this.context.stats.carregamento.falha || 0,
                tempoOperacao: duration,
                // Não há mais batchResult direto aqui para retornar
            };
        }
        catch (error) {
            const duration = Date.now() - inicioCarregamento;
            this.context.logger.error(`❌ Erro no carregamento: ${error.message}`);
            // Atualizar estatísticas de falha no contexto
            const operacoesPendentes = (this.rawPerfisCompletos?.length || 0) + data.perfis.length + 1;
            this.context.stats.carregamento.falha += operacoesPendentes - this.context.stats.carregamento.sucesso;
            return {
                sucessos: this.context.stats.carregamento.sucesso,
                falhas: this.context.stats.carregamento.falha,
                tempoOperacao: duration,
                erro: error.message
            };
        }
...
exports.PerfilDeputadosProcessor = PerfilDeputadosProcessor;
//# sourceMappingURL=perfil-deputados.processor.js.map