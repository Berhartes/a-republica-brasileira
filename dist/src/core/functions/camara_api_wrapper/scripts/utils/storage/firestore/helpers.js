...
exports.parseFirestorePath = parseFirestorePath;
/**
...
function parseFirestorePath(collectionPath, documentId) {
    ...
    if (cleanedCollectionPath === '') {
        ...
        logging_1.logger.error(errorMsg, { originalPath: collectionPath });
        throw new Error(errorMsg);
    }
    if (cleanedDocumentId === '') {
        const errorMsg = 'ID do documento resultante inválido após limpeza.';
        logging_1.logger.error(errorMsg, { originalId: documentId });
        throw new Error(errorMsg);
    }
