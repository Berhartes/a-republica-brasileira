...
async function obterLegislaturaAtual(data) {
    ...
    try {
        ...
        const endpoint = api.replacePath(api_1.endpoints.LEGISLATURAS.LISTA.PATH, { data }); // Corrigido para LISTA
        // Fazer a requisição
        // O endpoint LISTA espera o parâmetro 'data' para filtrar
        const params = { ...api_1.endpoints.LEGISLATURAS.LISTA.PARAMS, data };
        const response = await (0, logging_2.withRetry)(async () => api.get(endpoint, params), // Corrigido para LISTA e passando params com data
        api_1.endpoints.REQUEST.RETRY_ATTEMPTS, api_1.endpoints.REQUEST.RETRY_DELAY, `Extração legislatura atual para data ${data}`);
