...
const config_1 = require("../utils/storage/firestore/config"); // Admin SDK
const args_parser_1 = require("../utils/cli/args-parser");
const perfildeputados_1 = require("../extracao/perfildeputados");
const error_handler_1 = require("../utils/logging/error-handler");
const endpoints_1 = require("../config/endpoints"); // Adicionado
const api = tslib_1.__importStar(require("../utils/api")); // Adicionado
...
class DiscursosExtractor {
    ...
    async extractDiscursosAtualizacao(deputadoId, legislatura) {
        try {
            logger_1.logger.info(`Iniciando atualização incremental de discursos do deputado ${deputadoId}`);
            ...
            if (discursosNovos.length === 0) {
                logger_1.logger.info(`✅ Deputado ${deputadoId} já está atualizado - nenhum novo discurso nos últimos 60 dias`);
                return {
            ...
            logger_1.logger.info(`🔄 Deputado ${deputadoId}: encontrados ${discursosNovos.length} novos discursos dos últimos 60 dias`);
            ...
            logger_1.logger.error(`Erro na atualização incremental de discursos do deputado ${deputadoId}: ${error.message}`);
            return {
                deputadoId: deputadoId.toString(),
                totalDiscursos: 0,
                totalPaginas: 0,
    ...
    async verificarDadosExistentes(deputadoId) {
        try {
            const discursosArrayRef = config_1.db.doc(`congressoNacional/camaraDeputados/discursos/${deputadoId}/dados/items`);
            const doc = await discursosArrayRef.get();
            if (!doc.exists) {
                logger_1.logger.info(`Deputado ${deputadoId} não tem discursos no Firestore - processará todos os discursos recentes`);
                return { discursosExistentes: [] };
            }
            const dados = doc.data();
            const discursosExistentes = (dados?.items || []).map((item) => item.id);
            logger_1.logger.info(`Deputado ${deputadoId} tem ${discursosExistentes.length} discursos já processados`);
            return { discursosExistentes };
        }
        catch (error) {
            logger_1.logger.warn(`Erro ao verificar dados existentes do deputado ${deputadoId}: ${error.message}`);
            return { discursosExistentes: [] };
        }
    }
    /**
    ...
    calcularDataLimite60Dias() {
        ...
        logger_1.logger.info(`Data limite para discursos: ${dataLimite.toISOString().split('T')[0]}`);
        return dataLimite;
    }
    /**
    ...
    async filtrarDiscursosNovos(discursosRecentes, idsExistentes) {
        ...
        logger_1.logger.info(`Filtrados ${discursosNovos.length} discursos novos de ${discursosRecentes.length} discursos recentes`);
    ...
    async extractDiscursosCompletos(deputadoId, legislatura) {
        try {
            logger_1.logger.info(`Iniciando extração completa de discursos do deputado ${deputadoId}`);
            ...
            while (temMaisPaginas) {
                try {
                    logger_1.logger.info(`Extraindo página ${paginaAtual} de discursos do deputado ${deputadoId}`);
                    ...
                    logger_1.logger.info(`🔍 Parâmetros da requisição: ${JSON.stringify(params, null, 2)}`);
                    // Fazer requisição com retry
                    logger_1.logger.info(`🔍 DEBUG: URL completa: ${endpoints_1.endpoints.BASE_URL}${endpoint}?${new URLSearchParams(params).toString()}`);
                    const response = await (0, error_handler_1.withRetry)(async () => api.get(endpoint, params), endpoints_1.endpoints.REQUEST.RETRY_ATTEMPTS, endpoints_1.endpoints.REQUEST.RETRY_DELAY, `Extração discursos do deputado ${deputadoId} - página ${paginaAtual}`);
                    // Log de debug da resposta
                    logger_1.logger.info(`🔍 Resposta recebida: ${response ? 'SIM' : 'NÃO'}, dados: ${response?.dados ? response.dados.length : 'NENHUM'} itens`);
                    if (response?.links) {
                        logger_1.logger.info(`🔍 Links de paginação: ${JSON.stringify(response.links, null, 2)}`);
                    }
                    if (!response || !response.dados) {
                        logger_1.logger.warn(`Sem dados na página ${paginaAtual} para deputado ${deputadoId}`);
                        break;
                    }
                    const discursosPagina = response.dados;
                    logger_1.logger.info(`Encontrados ${discursosPagina.length} discursos na página ${paginaAtual}`);
                    // Se não há discursos nesta página, parar
                    if (!Array.isArray(discursosPagina) || discursosPagina.length === 0) {
                        logger_1.logger.info(`Página ${paginaAtual} vazia - finalizando extração`);
                        break;
                    }
                    ...
                    if (discursosPagina.length < ITENS_POR_PAGINA) {
                        logger_1.logger.info(`Página ${paginaAtual} retornou ${discursosPagina.length} itens (< ${ITENS_POR_PAGINA}) - última página`);
                    ...
                    logger_1.logger.error(`Erro ao extrair página ${paginaAtual} do deputado ${deputadoId}: ${error.message}`);
                    ...
                    if (error.message?.includes('404') || error.message?.includes('Página não encontrada')) {
                        logger_1.logger.info(`Página ${paginaAtual} não encontrada - finalizando extração`);
                        break;
                    }
                    // Para outros erros, tentar próxima página (pode ser erro temporário)
                    logger_1.logger.warn(`Erro na página ${paginaAtual}, tentando próxima...`);
                    paginaAtual++;
                    // Mas não continuar indefinidamente
                    if (paginaAtual > 50) { // Limite de segurança
                        logger_1.logger.warn(`Muitas páginas processadas (${paginaAtual}), parando por segurança`);
                        break;
                    }
                }
            }
            ...
            logger_1.logger.info(`Extração completa finalizada para deputado ${deputadoId}: ${todosDiscursos.length} discursos em ${totalPaginas} páginas`);
            return {
                deputadoId: deputadoId.toString(),
            ...
            logger_1.logger.error(`Erro na extração completa de discursos do deputado ${deputadoId}: ${error.message}`);
            return {
                deputadoId: deputadoId.toString(),
                totalDiscursos: 0,
                totalPaginas: 0,
...
function detectarLegislatura(args) {
    ...
    for (const arg of process.argv) {
        ...
        if (match) {
            ...
            if (numLegislatura >= 50 && numLegislatura <= 60) {
                ...
                logger_1.logger.info(`🏛️ Legislatura detectada via argumento numérico (--${legislatura}): ${legislatura}ª Legislatura`);
                return legislatura;
            }
            else if (numLegislatura >= 1 && numLegislatura <= 100) {
                // Range mais amplo mas com aviso (para compatibilidade histórica)
                logger_1.logger.warn(`⚠️ Legislatura ${numLegislatura} fora do range comum (50-60), mas será aceita`);
                legislatura = numLegislatura;
                logger_1.logger.info(`🏛️ Legislatura detectada via argumento numérico (--${legislatura}): ${legislatura}ª Legislatura`);
                return legislatura;
            }
            else {
                logger_1.logger.error(`❌ Legislatura inválida: ${numLegislatura}. Para a Câmara dos Deputados, use valores entre 50 e 60.`);
                process.exit(1);
            }
        }
    }
    ...
    if (args.legislatura !== undefined) {
        ...
        if (legislaturaArg >= 1 && legislaturaArg <= 100) {
            ...
            logger_1.logger.info(`🏛️ Legislatura detectada via parâmetro --legislatura: ${legislatura}ª Legislatura`);
            return legislatura;
        }
        else {
            logger_1.logger.error(`❌ Legislatura inválida: ${args.legislatura}. Deve ser um número entre 1 e 100.`);
            process.exit(1);
        }
    }
    ...
    if (primeiroArg && !primeiroArg.startsWith('-') && !isNaN(parseInt(primeiroArg, 10))) {
        ...
        if (legislaturaArg >= 1 && legislaturaArg <= 100) {
            legislatura = legislaturaArg;
            logger_1.logger.info(`🏛️ Legislatura detectada via primeiro argumento: ${legislatura}ª Legislatura`);
            return legislatura;
        }
        else {
            logger_1.logger.error(`❌ Legislatura inválida: ${legislaturaArg}. Deve ser um número entre 1 e 100.`);
            process.exit(1);
        }
    }
    // Se chegou aqui, usar legislatura padrão
    logger_1.logger.info(`🏛️ Usando legislatura padrão: ${legislatura}ª Legislatura (${LEGISLATURA_ATUAL})`);
    return legislatura;
}
/**
...
async function processarDiscursosDeputados() {
    try {
        ...
        const args = (0, args_parser_1.parseArgs)();
        ...
        logger_1.logger.info(`Iniciando processamento de discursos de deputados da legislatura ${legislatura}`);
        logger_1.logger.info(`Configurações: limite=${limite}, concorrência=${concorrencia}, modo=${modoAtualizacao ? 'ATUALIZAÇÃO INCREMENTAL (60 dias)' : 'COMPLETO'}`);
        logger_1.logger.info(`${modoAtualizacao ? '📋 Modo Atualização: Processará apenas discursos novos dos últimos 60 dias' : '📋 Modo Completo: Processará todos os discursos disponíveis'}`);
        logger_1.logger.info(`🏛️ Processando discursos da ${legislatura}ª Legislatura`);
        // Inicializar extratores
        const perfilExtractor = new perfildeputados_1.PerfilDeputadosExtractor();
        const discursosExtractor = new DiscursosExtractor();
        // 1. Extrair lista de deputados da legislatura
        logger_1.logger.info('Etapa 1: Extraindo lista de deputados');
        ...
        if (!listaDeputados.deputados || listaDeputados.deputados.length === 0) {
            logger_1.logger.error(`Nenhum deputado encontrado para a legislatura ${legislatura}`);
            return;
        }
        logger_1.logger.info(`Encontrados ${listaDeputados.deputados.length} deputados na legislatura ${legislatura}`);
        // Aplicar limite se especificado
        let deputadosParaProcessar = listaDeputados.deputados;
        if (limite > 0 && limite < deputadosParaProcessar.length) {
            logger_1.logger.info(`Aplicando limite de ${limite} deputados`);
        ...
        logger_1.logger.info(`Etapa 2: Extraindo ${tipoProcessamento} de ${deputadosParaProcessar.length} deputados`);
        ...
        for (let i = 0; i < idsDeputados.length; i += concorrencia) {
            ...
            logger_1.logger.info(`Processando lote ${Math.floor(i / concorrencia) + 1} com ${lote.length} deputados`);
            ...
            const promessas = lote.map(async (idDeputado) => {
                try {
                    ...
                    if (resultadoDiscursos.erro) {
                        logger_1.logger.error(`Erro na extração de discursos do deputado ${idDeputado}: ${resultadoDiscursos.erro}`);
                        deputadosComErro++;
                        return null;
                    }
                    ...
                    if (resultadoDiscursos.totalDiscursos === 0) {
                        if (modoAtualizacao) {
                            logger_1.logger.info(`✅ Deputado ${idDeputado}: atualizado - nenhum novo discurso nos últimos 60 dias`);
                            deputadosJaAtualizados++;
                        }
                        else {
                            logger_1.logger.info(`ℹ️ Deputado ${idDeputado}: nenhum discurso encontrado`);
                        }
                    ...
                    logger_1.logger.info(`✅ Deputado ${idDeputado}: ${resultadoDiscursos.totalDiscursos} ${modoTexto} em ${resultadoDiscursos.totalPaginas} páginas`);
                    return resultadoDiscursos;
                }
                catch (error) {
                    logger_1.logger.error(`Erro ao processar deputado ${idDeputado}: ${error.message}`);
                    deputadosComErro++;
                    return null;
                }
            });
            ...
            // Log de progresso
            logger_1.logger.info(`Progresso: ${Math.min(i + concorrencia, idsDeputados.length)}/${idsDeputados.length} deputados processados`);
            // Delay entre lotes para não sobrecarregar a API
            if (i + concorrencia < idsDeputados.length) {
                logger_1.logger.info('Aguardando 5 segundos antes do próximo lote...');
                await new Promise(resolve => setTimeout(resolve, 5000));
            }
        }
        // 3. Resumo final
        logger_1.logger.info('====================================================');
        logger_1.logger.info(`📊 RESUMO DO PROCESSAMENTO DE DISCURSOS`);
        logger_1.logger.info('====================================================');
        logger_1.logger.info(`🔧 Modo: ${modoAtualizacao ? 'ATUALIZAÇÃO INCREMENTAL (últimos 60 dias)' : 'PROCESSAMENTO COMPLETO'}`);
        logger_1.logger.info(`✅ Deputados processados com sucesso: ${deputadosProcessados}`);
        if (modoAtualizacao && deputadosJaAtualizados > 0) {
            logger_1.logger.info(`🔄 Deputados já atualizados (sem novos dados): ${deputadosJaAtualizados}`);
        }
        logger_1.logger.info(`❌ Deputados com erro: ${deputadosComErro}`);
        logger_1.logger.info(`🎤 Total de ${modoAtualizacao ? 'novos ' : ''}discursos extraídos: ${totalDiscursosExtraidos.toLocaleString()}`);
        if (modoAtualizacao) {
            logger_1.logger.info(`📅 Período verificado: últimos 60 dias`);
        }
        logger_1.logger.info('====================================================');
        logger_1.logger.info('Processamento de discursos de deputados concluído com sucesso');
    }
    catch (error) {
        logger_1.logger.error(`Erro no processamento de discursos de deputados: ${error.message}`);
...
async function salvarDiscursosFirestore(resultadoDiscursos, modoAtualizacao = false) {
    try {
        ...
        logger_1.logger.info(`Salvando ${totalDiscursos} ${modoAtualizacao ? 'novos ' : ''}discursos do deputado ${deputadoId} no Firestore`);
        // Referência do documento principal
        const deputadoDiscursosRef = config_1.db.doc(`congressoNacional/camaraDeputados/discursos/${deputadoId}`);
        const discursosArrayRef = config_1.db.doc(`congressoNacional/camaraDeputados/discursos/${deputadoId}/dados/items`);
        let dadosFinais;
        let discursosFinais;
        if (modoAtualizacao && totalDiscursos > 0) {
            // MODO ATUALIZAÇÃO: Mesclar com dados existentes
            logger_1.logger.info(`Modo atualização: mesclando ${totalDiscursos} novos discursos com dados existentes`);
            ...
            logger_1.logger.info(`Dados existentes: ${discursosExistentes.length} discursos`);
            ...
            logger_1.logger.info(`Dados finais: ${discursosFinais.length} discursos totais`);
        }
        else {
            // MODO COMPLETO: Substituir todos os dados
            logger_1.logger.info(`Modo completo: substituindo todos os dados`);
        ...
        logger_1.logger.info(`✅ Discursos do deputado ${deputadoId} salvos com sucesso (${modoAtualizacao ? 'modo incremental' : 'modo completo'})`);
    }
    catch (error) {
        logger_1.logger.error(`Erro ao salvar discursos do deputado ${resultadoDiscursos.deputadoId}: ${error.message}`);
        throw error;
    }
}
// Executar função principal
processarDiscursosDeputados().catch(error => {
    logger_1.logger.error(`Erro fatal no processamento: ${error.message}`);
    logger_1.logger.error(error.stack);
    process.exit(1);
});
