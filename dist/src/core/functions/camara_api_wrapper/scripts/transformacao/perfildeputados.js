...
exports.PerfilDeputadosTransformer = void 0;
/**
 ...
 */
class PerfilDeputadosTransformer {
    /**
    ...
    transformDeputadosLegislatura(extractionResult, legislaturaNumero) {
        logger_1.logger.info('Transformando dados de deputados de legislatura específica');
        const { deputados, metadados } = extractionResult;
        if (!deputados || !Array.isArray(deputados) || deputados.length === 0) {
            logger_1.logger.warn('Dados de deputados inválidos ou vazios para transformação');
            return {
        ...
        const deputadosTransformados = deputados.map(deputado => {
            try {
                ...
                logger_1.logger.warn(`Erro ao transformar deputado básico: ${error.message}`);
        ...
        logger_1.logger.info(`Transformados ${deputadosTransformados.length} deputados da legislatura ${legislatura}`);
        return {
            timestamp: new Date().toISOString(),
    ...
    transformDeputadoBasico(deputado) {
        ...
        if (!deputado || !deputado.id) {
            logger_1.logger.warn('Dados incompletos de deputado para transformação básica');
            return null;
        }
    ...
    transformPerfilCompleto(perfilCompleto) {
        try {
            ...
            if (!perfilCompleto) {
                logger_1.logger.error(`Perfil completo é nulo ou indefinido`);
                return null;
            }
            ...
            if (!perfilCompleto.dadosBasicos ||
                ...
                logger_1.logger.warn(`Dados básicos incompletos ou vazios para o deputado ${perfilCompleto.codigo || 'desconhecido'}`);
            ...
            logger_1.logger.info(`Transformando perfil completo do deputado ${perfilCompleto.codigo}`);
            ...
            if (!deputadoBasico) {
                logger_1.logger.warn(`Não foi possível transformar os dados básicos do deputado ${perfilCompleto.codigo}`);
                return null;
            }
            ...
            logger_1.logger.error(`Erro ao transformar perfil completo do deputado ${perfilCompleto?.codigo || 'desconhecido'}: ${error.message}`);
            return null;
        }
    }
}
exports.PerfilDeputadosTransformer = PerfilDeputadosTransformer;
